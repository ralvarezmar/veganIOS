# Command for library generation

The `afe new` command contemplates the generation of an **library** project. To take advantage of the feature, simply add the parameter `--type=library` to the command.
...
To start a new workspace with the structure of a library, simply run the command:

```bash title='Terminal'
afe new <PROJECT_NAME> --type=library --prefix=<SIGLA_DO_PROJETO>
```
