...
``` ts
constructor(private readonly _routerFromChild?: any)
```
...
|---|---|
| `_routerFromChild`: any | Optional. Angular Router from child component |
...
Then, it will try to load the microfront as a module using the technical grouping and the version.
In case of any error, the [`loadRemoteError()`](microfrontcontainerdirective.md#loadremoteerror) method will be invoked.
...
export class ExampleComponent extends MicrofrontContainerDirective {
  constructor(private _router: Router) {
    ...
    **/
    const myProps: MyProps | undefined = thisgetMicrofrontNavigationProps<MyProps>();
    if (myProps) {
