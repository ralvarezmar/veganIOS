# How to transform a Darwin Classic SPA into a Darwin Gluon SPA

A Darwin Classic SPA is a Single Page Application that uses the classic Darwin modules:
...
This way, a Classic SPA follows the classic corporative security using BKS tokens.

A Darwin Gluon SPA is a Single Page Application that uses the new security approach for Gluon based on OAuth 2.0 protocols (Plain Oauth or OIDC),
...
A Darwin Gluon SPA will use the following modules:
...
To transform a Darwin Classic SPA into a Darwin Gluon SPA, you must follow the next steps:
...
@Component({
  ...
  template: `
    @let isSessionInitialized = securityService.onSessionInitialized$ | async;
    @if (isSessionInitialized === undefined) {
      <app-example />
    }
  `,
})
...
!!! note
    ...
    Please note, at this point, you are configuring the OAuth server so you'll need to have an OAuth server running and configured to work with your SPA.
...
export const routes: Routes = [
  {
    ...
    component: YourSecuredComponent
  },
  ···
];
```
...
### 7. Ensure you are using Puppeteer to run your tests
...
The next step is to configure and use the reusable component library we have installed: Flame User Interface for Angular (`@santander/flame-ui-angular`).

We'll need to configure the Flame providers from (`FlameStencilAngularModule`) into your config file.
