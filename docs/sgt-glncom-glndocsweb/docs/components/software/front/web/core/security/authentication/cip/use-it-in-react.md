...
- Authentication: Provide CIP authorization flow (including implicit login)
- Logout features
...
And then install the library in the application running `npm install @santander/security-react`
...
export type RemoteAdapter = {
  ...
  /**
   * The instance of the http library for making requests. Can be an instance of FetchManager(@santander/http - by default) or XHR
   * (@gruposantander/web-ui-framework)
   */
...
To load previous configurations in the `main.tsx`, the `SCMCipProvider` is added in the `imports` section and initialized wrapping the `App` component.

```typescript
import { TokenStoreStrategyOptions } from '@santander/security';
import { SCMCipProvider } from '@santander/security-react';
```
...
## Protecting your inner routes: withCipAuthGuard

You can protect a route to avoid the user access a route without previous authentication. This is possible using with the HOC `withCipAuthGuard` passing the component that you want to protect.
If the user is not authenticated, will emit an `SCMEvent` of type `GLN_SCM_NOT_AUTHENTICATED`.

Example:

``` tsx
import { withCipAuthGuard, useScmCipAuth } from '@santander/security-react';
...
const AppRouter: React.FC = (): JSX.Element => {
  return (
    <Routes>
      ...
      <Route path="/home" Component={withCipAuthGuard(GlobalPosition)} />
...
Instead of using the guard explained before, it is possible using the hook `useScmCipAuth` in the component and to initiate the login process manually calling the `init()` method.

```tsx
import { useScmCipAuth } from '@santander/security-react';

const { initialize, validateCredentials, logout } = useScmCipAuth();
...
Two public methods are available for managing external tokens: `setToken` and `removeToken`. Only services configured as `isCoexistance` can be managed this way.
...
1 - In the app's `App.tsx` file, import the hook `useScmCipAuth`, extract `scmEvents$`, create a function and subscribe to `scmEvents$` observable and handle events as desired.
...
...
const { getCurrentState, onStateChange } = useScmCipAuth();

getCurrentState(): StateGeneric
onStateChange(callback: (state: StateGeneric) => void): void
```
