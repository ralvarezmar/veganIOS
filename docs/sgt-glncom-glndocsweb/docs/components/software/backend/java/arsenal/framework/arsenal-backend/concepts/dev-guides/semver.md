...
!!! warning
    ...
    The correction component must be reset when incrementing the
...
!!! warning
    ...
    Correction and functionality components must be reset when
...
The version must be completed using the <span
style="color:orange">version</span> tag present in the pom.xml file, as shown in
the example below:
