# Darwin Spring Boot Events ![5.8.2](https://img.shields.io/badge/5.8.2-FF073D)
...
For more details on batch consumption you can refer to the [spring-kafka documentation on batch listeners](https://docs.spring.io/spring-kafka/docs/3.2.9/reference/html/#batch-listeners).

### Health Checks Kafka

The event module auto-configures a `HealthIndicator` that allows to configure the [`Kubernetes probes`](https://docs.spring.io/spring-boot/docs/3.3.12/reference/html/actuator.html#production-ready-kubernetes-probes) depending on the connectivity with
Kafka.

This indicator makes use of [Spring Boot Actuator](https://docs.spring.io/spring-boot/docs/3.3.12/reference/html/actuator.html#production-ready) to add a new component under the health endpoint `http://localhost:8080/actuator/health/kafka`, whose
...
`kafka` cluster, since they are based on the [events that Spring Boot launches](https://docs.spring.io/spring-boot/docs/3.3.12/reference/html/#production-ready-kubernetes-probes-lifecycle) during the life cycle of the application, therefore, it is
...
Allows generating an activity log for consumed, processed and sent events.

Currently, there are three activity logs types:

- "receive": Log generated when an event is received.
- "process": Log generated when an event is processed.
- "send": Log generated when an event is sent.

!!! note

    In the case that the Kafka consumer is in batch mode, a log will be generated for each received event, but since
    they will be processed together, only one log will be generated for each processed event, with the `traceId` of the
    first event.

!!! note

    Before version 5.2.0 the activity logs "process" were call "poll"

This functionality can be disabled by setting the property `darwin.logging.observability.events.enabled` to false.
...
|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------|-------------------------------------------------------------------------------------------------------------|------------------------------------------|
| [DarwinCloudEvent](https://gluon.dev.corp/microservices/docs/darwin-spring-boot/5.8.2/apidocs/com/santander/darwin/events/DarwinCloudEvent.html)                                  | DarwinCloudEvent                | Exposes a Builder that allows us to create events of type DarwinCloudEvent.                                 | <ul><li>All</li></ul>                    |
| [InterceptorProducer](https://gluon.dev.corp/microservices/docs/darwin-spring-boot/5.8.2/apidocs/com/santander/darwin/events/interceptor/InterceptorProducer.html)                | ProducerInterceptor&lt;K, V&gt; | Interceptor associated to the producer of CloudEvent type messages in the topic.                            | <ul><li>All</li></ul>                    |
| [DarwinExtension](https://gluon.dev.corp/microservices/docs/darwin-spring-boot/5.8.2/apidocs/com/santander/darwin/events/extension/DarwinExtension.html)                          | Extension                       | Extension of CloudEvent for attributes specific to Darwin and which are not part of the standard CloudEvent | <ul><li>All</li></ul>                    |
| [CloudEventAvroSerializer](https://gluon.dev.corp/microservices/docs/darwin-spring-boot/5.8.2/apidocs/com/santander/darwin/events/serializers/CloudEventAvroSerializer.html)      | Serializer&lt;CloudEvent&gt;    | Serializer that allows to serialize CloudEvent events in Avro format.                                       | <ul><li>All</li></ul>                    |
| [CloudEventAvroDeserializer](https://gluon.dev.corp/microservices/docs/darwin-spring-boot/5.8.2/apidocs/com/santander/darwin/events/serializers/CloudEventAvroDeserializer.html)  | Serializer&lt;CloudEvent&gt;    | Deserializer that allows to deserialize CloudEvent events in Avro format.                                   | <ul><li>All</li></ul>                    |
| [AvroCloudEventData](https://gluon.dev.corp/microservices/docs/darwin-spring-boot/5.8.2/apidocs/com/santander/darwin/events/data/AvroCloudEventData.html)                         | CloudEventData                  | Class that allows encapsulating Avro types as CloudEventData.                                               | <ul><li>All</li></ul>                    |
| [ReactiveContextUtil](https://gluon.dev.corp/microservices/docs/darwin-spring-boot/5.8.2/apidocs/com/santander/darwin/events/reactive/ReactiveContextUtil.html)                   | ReactiveContextUtil             | Bean that allows to work with Darwin context and security context over a reactive stream in a simple way.   | <ul><li>Reactive</li></ul>               |
...
| [KafkaHealthIndicator](https://gluon.dev.corp/microservices/docs/darwin-spring-boot/5.8.2/apidocs/com/santander/darwin/events/actuate/KafkaHealthIndicator.html)                  | HealthIndicator                 | Class that defines the Health strategy for Kafka brokers.                                                   | <ul><li>All</li></ul>                    |
...
!!! info "Important"
    ...
    [Spring Kafka](https://docs.spring.io/spring-kafka/docs/3.2.9/reference/html) provides several options to generate or consume messages, all of them valid together with `CloudEvent`, so here we will put a basic example based
...
To send a `DarwinCloudEvent` to a kafka topic in `Avro` format we will require the use of an [`Avro Schema`](https://avro.apache.org/docs/1.11.4/specification) associated to the information to be sent, this will imply the execution of a series of
...
!!! info "Important"
    ...
    [Spring Kafka](https://docs.spring.io/spring-kafka/docs/3.2.9/reference/html) provides several options to generate or consume messages, all of them valid together with `CloudEvent`, so here we will put a basic example, but
...
[ReactiveContextUtil](https://gluon.dev.corp/microservices/docs/darwin-spring-boot/5.8.2/apidocs/com/santander/darwin/events/reactive/ReactiveContextUtil.html) class exposes two static methods that extract them,
...
The [ReactiveContextUtil](https://gluon.dev.corp/microservices/docs/darwin-spring-boot/5.8.2/apidocs/com/santander/darwin/events/reactive/ReactiveContextUtil.html)
...
!!! note
    ...
    [ReactiveContextUtil](https://gluon.dev.corp/microservices/docs/darwin-spring-boot/5.8.2/apidocs/com/santander/darwin/events/reactive/ReactiveContextUtil.html) utility class. This allows to subscribe the Reactor
...
[ReactiveContextUtil](https://gluon.dev.corp/microservices/docs/darwin-spring-boot/5.8.2/apidocs/com/santander/darwin/events/reactive/ReactiveContextUtil.html) class exposes a method that is responsible for
...
5. The ***fillContext*** method of the [ReactiveContextUtil](https://gluon.dev.corp/microservices/docs/darwin-spring-boot/5.8.2/apidocs/com/santander/darwin/events/reactive/ReactiveContextUtil.html) class is used
