...
- Removed line that verify bean  `swaggerConfig` in `SwaggerConfigTest` class. In Spring 3.4 the configuration class bean name changed.
In this case the bean `swaggerConfig` is called `${package}.SwaggerConfig`, to avoid issues we remove the test.
