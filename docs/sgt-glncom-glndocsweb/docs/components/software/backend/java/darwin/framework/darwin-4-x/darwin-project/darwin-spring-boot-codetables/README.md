# Darwin Spring Boot Code Tables ![4.3.3-RELEASE](https://img.shields.io/badge/4.3.3-RELEASE-FF073D)
...
The library will configure itself depending on the environment in which the application is located.

The types of application that the library is capable of detecting are:

- ***NotWeb***: The application does not run as a web application and therefore **should not** start an embedded web server.
...
- ***NotWeb***: base configuration that loads both implementations of the library.

- ***Servlet***: Servlet web components[^1]

- ***Reactive***: Reactive web components[^2]
...
A `Bean` that implements the `CodeTablesService` interface is configured and exposed. It provides a series of methods that allow access to each of the endpoints of the Code Tables service in a simple way.
...
To use the library, you must include the library's starter as a Maven dependency in the `pom.xml` file:
    ...
    <dependency>
        ...
        <artifactId>darwin-spring-boot-starter-code-tables</artifactId>
    </dependency>
...
[^1]: If we run a Web Servlet application, Only components of this type will be loaded (not reactivate them)

[^2]: If we run a Web Reactive application, Only the components of this type will be loaded (not the servlet)
