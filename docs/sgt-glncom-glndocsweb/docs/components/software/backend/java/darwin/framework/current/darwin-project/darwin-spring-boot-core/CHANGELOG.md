...
- Solve issue that avoid accessing to the form data body properly when the content type was 'application/form-urlencoded'.
...
- Fixed issue preventing `darwin.core.http-clients.netty-http-client` properties from being applied when `darwin.core.http-clients.apache-http-client`
  is unset.
...
- Fix [issue](https://github.com/spring-projects/spring-boot/issues/44342) related to tomcat 10.1.36 with Spring Boot 3.4.2.
...
- Be more tolerant in interceptors and filters when the context is empty and don't throw an exception.
...
- Darwin RestTemplate bean and WebClient bean injections are not configured by Darwin by default (use Builder beans instead).
  New properties to restore the old behavior `darwin.core.webclient.customize-beans` and `darwin.core.resttemplate.customize-beans`.
- New property to disable the RestTemplate configuration `darwin.core.resttemplate.enabled` (enabled by default).
- `CopyContextFilterFunction` has been deprecated and are no longer used by the framework.
- New method to access Darwin context `DarwinContextHolder#getCurrentContextIfPresent` and `DarwinContextHolder#getCurrentContextOrThrow`.
- New property `darwin.core.reactor.context-schedule-hook-enabled` that allow disable the onScheduleHook use to progress context to reactor schedulers.
- New properties `darwin.core.http-clients.apache-http-client` and `darwin.core.http-clients.netty-http-client` to configure the HttpClient used by RestTemplate, RestClient and WebClient.
- Configure HttpClient directly instead use customizer of RestTemplate, RestClient and WebClient. This allows setting up
  SSL configuration without losing Darwin customization.
- You could configure proxies for RestTemplate, RestClient and WebClient using the properties `darwin.core.proxy`.
...
- Placeholders replacement. Added the possibility to enrich errors with variables that are resolved at runtime.
