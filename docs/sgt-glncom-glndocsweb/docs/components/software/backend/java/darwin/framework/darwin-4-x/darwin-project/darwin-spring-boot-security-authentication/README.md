# Darwin Spring Boot Authentication ![4.3.3-RELEASE](https://img.shields.io/badge/4.3.3-RELEASE-FF073D)
...
The library will autoconfigure only those functionalities that apply to the environment in which the application is running. To define the execution environment of a `Spring Boot` application we will use the values defined in
[WebApplicationType](https://docs.spring.io/spring-boot/docs/3.1.12/api/org/springframework/boot/WebApplicationType.html), This can be defined by a property or in code:
...
- To do this through code we will make use of the class `SpringApplicationBuilder`:
...
- ***NotWeb***: The application does not run as a web application and therefore should not start an `embedded web server`. but **detects the presence of the dependency** `restTemplate` and `webClient` so \*they are auto-configured with the
    `Darwin` interceptors.
...
#### Get channel from JWT authentication token

If the microservice is called with a JWT token with the claim `channel_tp` its value is added to the context as a channel. If the channel is also informed as request, the token claim value has priority.
      ...
      security:
        oauth2:
          resourceserver:
            jwt:
              jwk-set-uri:
                - ${env.connectors.resourceserver.jwk-endpoint}
      ...
      security:
        oauth2:
          resourceserver:
            jwt:
              jwk-set-uri:
                - ${env.connectors.resourceserver.jwk-endpoint} # Define pkm-endpoint or jwk-set-uri but not both
...
For **Reactive application**:
    ...
    DarwinReactiveHttpSecurityCustomizer darwinAuthorizationConfiguration(
            ServerWebExchangeMatcher darwinMatcherWhiteList) {

        return http -> {
            // All requests must be authenticated and install authentication filter
            // If endpoint ends in info or health allow it
            http.authorizeExchange().matchers(darwinMatcherWhiteList).permitAll()
                    // Any other case is necessary to be authenticated
...
For **Servlet application**:
    ...
    DarwinHttpSecurityCustomizer darwinAuthorizationConfiguration() {
        ...
        return http -> {
            // Configure Exception handling with authenticationEntryPoint
            // All requests must be authenticated and install authentication filter
            http.antMatcher("/**").authorizeHttpRequests().anyRequest().authenticated();
        };
    }
    ...
    DarwinHttpSecurityCustomizer darwinAuthorizationConfiguration() {
        return http -> {
            http.authorizeHttpRequests(authorize -> authorize
                        .mvcMatchers("/application-servlet/hello").permitAll()
                        .mvcMatchers("/application-servlet/hello2").hasRole("ADMIN")
                        .mvcMatchers("/application-servlet/hello3").hasRole("USER")
                        .anyRequest().denyAll()
                );
        };
    }
...
[^1]: If you run a Web Servlet application, in addition to the own configuration for this environment, necessary components of the NotWeb configuration will be loaded

[^2]: If we run a Reactive web application, in addition to the configuration for this environment,
the necessary components of the NotWeb configuration will be loaded,
in addition to the configuration for this environment,
the necessary components of
the NotWeb configuration will be loaded
