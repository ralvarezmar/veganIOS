...
- Don't depend on `darwin-spring-boot-starter-logging` and `darwin-spring-boot-starter-authentication` anymore.
- Simplify events activity log. Just one activity trace by received event.
...
- New property `darwin.events.context-creation.enabled` to enable or disable the creation of a new session for each event.
- Added some pending hints to native compilation to avoid that users had to added manually.
...
- Business Events functionality has been migrated to its new own module
- Fix tracing error when authentication token has bad format.
- Fix error tracing consumed Kafka messages, don't lost consumed message processing span.
- Add traceability to kafka consumers in batch mode.
...
- Upgrade KafkaHealthContributorAutoConfig to support new interface in SB 3.2.
- Remove unnecessary cast to AuthenticationBearerToken in InterceptorProducer class.
