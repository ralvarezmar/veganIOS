# Darwin Spring Boot Cache ![4.3.3-RELEASE](https://img.shields.io/badge/4.3.3-RELEASE-FF073D)
...
By default, the protocol used is set to **2.9** for backward compatibility. It is recommended to upgrade the protocol,
you need to set the `infinispan.remote.protocol-version` parameter to 3.1 version in order to perform all capabilities.
...
The following table shows which RHDG (Red Hat JBoss Data Grid) version matches which HotRod protocol version:
...
All functionally related to Infinispan is not supported.
...
More information on how to use these annotations can be found in [cache-annotations](https://docs.spring.io/spring-framework/docs/6.0.21/reference/html/integration.html#cache-annotations)
...
More information on [Spring Boot - Caffeine](https://docs.spring.io/spring-boot/docs/3.1.12/reference/html/io.html#io.caching.provider.caffeine) and more detailed property specifications in
...
In case that the cache has not been activated or it is of "none" type, the service will remain available, but it will work simply by returning the object to be cached, without doing anything else.
