# Darwin Spring Boot Events ![4.3.3-RELEASE](https://img.shields.io/badge/4.3.3-RELEASE-FF073D)

![GA](https://img.shields.io/badge/GA-C81D11)

## Description

The `Darwin Spring Boot Events` library provides several functionalities depending on the configuration you want to load.

- On the one hand, it provides the necessary functionality to generate and/or consume events from `Kafka` topics.

- On the other hand, it provides the necessary functionality to generate `Business Events` that contain the functional information associated with a REST request. In addition, it will provide an annotation to mark public methods in the service
    layer components to generate these business events.
...
- The `Kafka Producer` is configured with the following properties:
    ...
    !!! info "Important"
        ...
        The ProducerInterceptor is only loaded in non-reactive KafkaProducer.
...
- `Kafka Consumer` is configured with the following properties:
    ...
    - A `ConsumerInterceptor` is added in charge of **renaming CloudEvents headers to their Sleuth equivalents** in addition to **populating the `DarwinInfo` object** from the `CloudEvent` event and **write activityLog**.

        !!! info "Important"

            The ProducerInterceptor is only loaded in non-reactive KafkaConsumer.
...
For more details on batch consumption you can refer to the [spring-kafka documentation on batch listeners](https://docs.spring.io/spring-kafka/docs/3.0.17/reference/html/#batch-listeners).

### Health Checks Kafka

The event module auto-configures a `HealthIndicator` that allows to configure the [`Kubernetes probes`](https://docs.spring.io/spring-boot/docs/3.1.12/reference/html/actuator.html#production-ready-kubernetes-probes) depending on the connectivity with
Kafka.

This indicator makes use of [Spring Boot Actuator](https://docs.spring.io/spring-boot/docs/3.1.12/reference/html/actuator.html#production-ready) to add a new component under the health endpoint `http://localhost:8080/actuator/health/kafka`, whose
...
`kafka` cluster, since they are based on the [events that Spring Boot launches](https://docs.spring.io/spring-boot/docs/3.1.12/reference/html/#production-ready-kubernetes-probes-lifecycle) during the life cycle of the application, therefore, it is
...
!!! info "Important"
    ...
    Once the onboarding has been done, it is necessary to create a topic with the Business Event scheme. [Documentation how to create a
    topic](https://sanes.atlassian.net/wiki/spaces/AVG/pages/24630198541/Kafka+Topology+Creator+Pipeline+Vostok-template) and [Business Event schema](#configure-business-events)
...
|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------|-------------------------------------------------------------------------------------------------------------|------------------------------------------|
| [DarwinCloudEvent](https://gluon.dev.corp/microservices/docs/darwin-spring-boot/4.3.3-RELEASE/apidocs/com/santander/darwin/events/DarwinCloudEvent.html)                                  | DarwinCloudEvent                | Exposes a Builder that allows us to create events of type DarwinCloudEvent.                                 | <ul><li>All</li></ul>                    |
| [InterceptorProducer](https://gluon.dev.corp/microservices/docs/darwin-spring-boot/4.3.3-RELEASE/apidocs/com/santander/darwin/events/interceptor/InterceptorProducer.html)                | ProducerInterceptor&lt;K, V&gt; | Interceptor associated to the producer of CloudEvent type messages in the topic.                            | <ul><li>All</li></ul>                    |
| [InterceptorConsumer](https://gluon.dev.corp/microservices/docs/darwin-spring-boot/4.3.3-RELEASE/apidocs/com/santander/darwin/events/interceptor/InterceptorConsumer.html)                | ConsumerInterceptor&lt;K, V&gt; | Interceptor associated with the consumption of messages of type CloudEvent from the topic.                  | <ul><li>All</li></ul>                    |
| [DarwinExtension](https://gluon.dev.corp/microservices/docs/darwin-spring-boot/4.3.3-RELEASE/apidocs/com/santander/darwin/events/extension/DarwinExtension.html)                          | Extension                       | Extension of CloudEvent for attributes specific to Darwin and which are not part of the standard CloudEvent | <ul><li>All</li></ul>                    |
| [CloudEventAvroSerializer](https://gluon.dev.corp/microservices/docs/darwin-spring-boot/4.3.3-RELEASE/apidocs/com/santander/darwin/events/serializers/CloudEventAvroSerializer.html)      | Serializer&lt;CloudEvent&gt;    | Serializer that allows to serialize CloudEvent events in Avro format.                                       | <ul><li>All</li></ul>                    |
| [CloudEventAvroDeserializer](https://gluon.dev.corp/microservices/docs/darwin-spring-boot/4.3.3-RELEASE/apidocs/com/santander/darwin/events/serializers/CloudEventAvroDeserializer.html)  | Serializer&lt;CloudEvent&gt;    | Deserializer that allows to deserialize CloudEvent events in Avro format.                                   | <ul><li>All</li></ul>                    |
| [AvroCloudEventData](https://gluon.dev.corp/microservices/docs/darwin-spring-boot/4.3.3-RELEASE/apidocs/com/santander/darwin/events/data/AvroCloudEventData.html)                         | CloudEventData                  | Class that allows encapsulating Avro types as CloudEventData.                                               | <ul><li>All</li></ul>                    |
| [ReactiveContextUtil](https://gluon.dev.corp/microservices/docs/darwin-spring-boot/4.3.3-RELEASE/apidocs/com/santander/darwin/events/reactive/ReactiveContextUtil.html)                   | ReactiveContextUtil             | Bean that allows to work with Darwin context and security context over a reactive stream in a simple way.   | <ul><li>Reactive</li></ul>               |
...
| [KafkaHealthIndicator](https://gluon.dev.corp/microservices/docs/darwin-spring-boot/4.3.3-RELEASE/apidocs/com/santander/darwin/events/actuate/KafkaHealthIndicator.html)                  | HealthIndicator                 | Class that defines the Health strategy for Kafka brokers.                                                   | <ul><li>All</li></ul>                    |
| [BusinessEventService](https://gluon.dev.corp/microservices/docs/darwin-spring-boot/4.3.3-RELEASE/apidocs/com/santander/darwin/events/business/BusinessEventService.html)                 | BusinessEventService            | Service to send business events in a blocking manner.                                                       | <ul><li>Servlet</li><li>NotWeb</li></ul> |
| [ReactiveBusinessEventService](https://gluon.dev.corp/microservices/docs/darwin-spring-boot/4.3.3-RELEASE/apidocs/com/santander/darwin/events/business/ReactiveBusinessEventService.html) | ReactiveBusinessEventService    | Service to send business events in a reactive manner.                                                       | <ul><li>All</li></ul>                    |
...
!!! info "Important"
    ...
    [Spring Kafka](https://docs.spring.io/spring-kafka/docs/3.0.17/reference/html) provides several options to generate or consume messages, all of them valid together with `CloudEvent`, so here we will put a basic example based
...
!!! info "Important"
    ...
    [Spring Kafka](https://docs.spring.io/spring-kafka/docs/3.0.17/reference/html) provides several options to generate or consume messages, all of them valid together with `CloudEvent`, so here we will put a basic example, but
...
[ReactiveContextUtil](https://gluon.dev.corp/microservices/docs/darwin-spring-boot/4.3.3-RELEASE/apidocs/com/santander/darwin/events/reactive/ReactiveContextUtil.html) class exposes two static methods that extract them,
...
The [ReactiveContextUtil](https://gluon.dev.corp/microservices/docs/darwin-spring-boot/4.3.3-RELEASE/apidocs/com/santander/darwin/events/reactive/ReactiveContextUtil.html)
...
!!! note
    ...
    [ReactiveContextUtil](https://gluon.dev.corp/microservices/docs/darwin-spring-boot/4.3.3-RELEASE/apidocs/com/santander/darwin/events/reactive/ReactiveContextUtil.html) utility class. This allows to subscribe the Reactor
...
[ReactiveContextUtil](https://gluon.dev.corp/microservices/docs/darwin-spring-boot/4.3.3-RELEASE/apidocs/com/santander/darwin/events/reactive/ReactiveContextUtil.html) class exposes a method that is responsible for
...
5. The ***fillContext*** method of the [ReactiveContextUtil](https://gluon.dev.corp/microservices/docs/darwin-spring-boot/4.3.3-RELEASE/apidocs/com/santander/darwin/events/reactive/ReactiveContextUtil.html) class is used
