# Darwin Spring Boot Microservice Archetype ![5.8.2](https://img.shields.io/badge/5.8.2-FF073D)
...
- [Arsenal JPA Code Generator](https://gluon.gs.corp/community/docs/latest/components/software/backend/java/arsenal/framework/arsenal-backend/tutorials/code-generator-jpa-maven-plugin/)
...
| ***batch-integration-version***       | The version of the Batch Accelerator to use. By default it uses the version specified in Darwin Spring Boot Dependencies                                                                                                                                         | Default: N                                                                                                                                             | False                               |
...
- ***webAppType***: Application type of the [mentioned above](#application-types) that we want to generate in relation to the values of the Spring Boot
      [WebApplicationType](https://docs.spring.io/spring-boot/docs/3.3.12/api/org/springframework/boot/WebApplicationType.html) property. of Spring Boot.
...
- ***batch-integration-version***: Indicates the version of the Batch Accelerator to use. By default, it uses the version specified in Darwin Spring Boot Dependencies.
...
```shell
mvn -B org.apache.maven.plugins:maven-archetype-plugin:3.3.1:generate -DarchetypeGroupId=com.santander.darwin -DarchetypeArtifactId=darwin-spring-boot-archetype-microservice -DarchetypeVersion=5.8.2 -Dcomponent-name=application-1 -Dacronym-app=myapps -DwebAppType=servlet
```
...
```shell
mvn -B org.apache.maven.plugins:maven-archetype-plugin:3.3.1:generate -DarchetypeGroupId=com.santander.darwin -DarchetypeArtifactId=darwin-spring-boot-archetype-microservice -DarchetypeVersion=5.8.2 -Dcomponent-name=applicationgluon -Dacronym-app=gluonapp -DwebAppType=reactive -Dgluon=Y -Dgluonlog-company=ESP -Dgluonlog-component-name=gluon-component -Dgluonlog-component-id=gluon-id -Dgluonlog-component-type=gluon-type -Dgluonlog-app-id=gluon-app-id
```
...
```shell
mvn -B org.apache.maven.plugins:maven-archetype-plugin:3.3.1:generate -DarchetypeGroupId=com.santander.darwin -DarchetypeArtifactId=darwin-spring-boot-archetype-microservice -DarchetypeVersion=5.8.2 -Dcomponent-name=application-1 -Dacronym-app=myapplication2 -DwebAppType=reactive -Dauthorization-library -Domnichannel-library -Dwebservice-component=Y
```
...
```shell
mvn -B org.apache.maven.plugins:maven-archetype-plugin:3.3.1:generate -DarchetypeGroupId=com.santander.darwin -DarchetypeArtifactId=darwin-spring-boot-archetype-microservice -DarchetypeVersion=5.8.2 -Dcomponent-name=application-1 -Dacronym-app=myapps -DwebAppType=servlet -Ddatabase-client=true -Ddatabase-client-type=postgresql
```
...
```shell
mvn -B org.apache.maven.plugins:maven-archetype-plugin:3.3.1:generate -DarchetypeGroupId=com.santander.darwin -DarchetypeArtifactId=darwin-spring-boot-archetype-microservice -DarchetypeVersion=5.8.2 -Dcomponent-name=applicationgluon -Dacronym-app=gluonapp -DwebAppType=reactive -Dgluon=Y -Dgluonlog-company=ESP -Dgluonlog-component-name=gluon-component -Dgluonlog-component-id=gluon-id -Dgluonlog-component-type=gluon-type -Dgluonlog-app-id=gluon-app-id -Dnative-compilation=true
```
...
```shell
mvn -B org.apache.maven.plugins:maven-archetype-plugin:3.3.1:generate -DarchetypeGroupId=com.santander.darwin -DarchetypeArtifactId=darwin-spring-boot-archetype-microservice -DarchetypeVersion=5.8.2 -Dcomponent-name=application-2 -Dacronym-app=myapps -DwebAppType=reactive -Dapi-first
```
...
```shell
mvn -B org.apache.maven.plugins:maven-archetype-plugin:3.3.1:generate -DarchetypeGroupId=com.santander.darwin -DarchetypeArtifactId=darwin-spring-boot-archetype-microservice -DarchetypeVersion=5.8.2 -Dcomponent-name=application-2 -Dacronym-app=myapps -DwebAppType=reactive -Dapi-first -Dapi-spec-url='https://catalog-api/domain/resources/openapi.yaml'
```
...
```shell
mvn -B org.apache.maven.plugins:maven-archetype-plugin:3.3.1:generate -DarchetypeGroupId=com.santander.darwin -DarchetypeArtifactId=darwin-spring-boot-archetype-microservice -DarchetypeVersion=5.8.2 -Dcomponent-name=application-1 -Dacronym-app=myapps -DwebAppType=reactive -Dgraphql
```
...
```shell
mvn -B org.apache.maven.plugins:maven-archetype-plugin:3.3.1:generate -DarchetypeGroupId=com.santander.darwin -DarchetypeArtifactId=darwin-spring-boot-archetype-microservice -DarchetypeVersion=5.8.2 -Dcomponent-name=Application-1 -Dacronym-app=myapps -DwebAppType=notweb
...
- A `infra` with a `service` package for the DatabaseService and its implementation.
- A `repository` package with a **UserRepository.java** interface that extends JpaRepository, plus a method to find a particular field in the Database.
- A `model` package with the **UserRecord.java** record to avoid exposing JPA entities.
- A `web` package with a **DatabaseController.java** class that implements two Servlet controller for creating and retrieving data from the database.

Also, a **HelloController.java** class that implements a  (`Servlet`) controller which returns `Hello World!' when an HTTP GET request is made to the endpoint _/postgresql-app/hello_.
...
- In pom.xml
  - Related to logging dependencies:
    - Don't include the logging based in log4j dependencies `darwin-spring-boot-starter-logging-basic` or `darwin-spring-boot-starter-logging-kafka`.
    - Exclude from `darwin-spring-boot-starter-events` the `darwin-spring-boot-starter-logging` dependency
    - Added profile `default` to include the `darwin-spring-boot-starter-logging-basic` dependency, based in logback. This profile is added automatically in all cases but when `nativeTest` is used.
    - We can't use logback for test because [Spring Boot Issue 36997](https://github.com/spring-projects/spring-boot/issues/36997), then for profile 'nativeTest' we use `slf4j-simple` for logging.
The 'nativeTest' profile has to uses only to check tests in native mode.
...
It also has an optional property called `darwin.logging.console-log-format` that allows to change the technical console log format.
