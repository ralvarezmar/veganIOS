# Darwin Spring Boot Library Archetype ![4.3.3-RELEASE](https://img.shields.io/badge/4.3.3-RELEASE-FF073D)
...
This project will serve as Parent POM for the Santander Java libraries based on Darwin, providing dependencies and plugin management.
...
If valid values are used in the parameters, the expected project will be generated without any problem:
    ...
    mvn -B archetype:generate -DarchetypeGroupId=com.santander.darwin -DarchetypeArtifactId=darwin-spring-boot-archetype-library -DarchetypeVersion=4.0.0-RELEASE -Dcomponent-name=greetings -Dacronym-app=darwin
...
For example, the parameter ***component-name*** must comply with the following validation: "*begin with a lowercase letter and continue with lowercase letters, numbers or hyphens, with a maximum of 33 characters*". If we use the value "Greeting$"
the following will occur:

    mvn -B archetype:generate -DarchetypeGroupId=com.santander.darwin -DarchetypeArtifactId=darwin-spring-boot-archetype-library -DarchetypeVersion=4.0.0-RELEASE -Dcomponent-name=Greeting$ -Dacronym-app=darwin
    [INFO] Scanning for projects...
    [INFO]
    [INFO] ------------------< org.apache.maven:standalone-pom >-------------------
    [INFO] Building Maven Stub Project (No POM) 1
...
- In the `main` package, all the classes and files with the library functionality will be included:
    ...
    - **spring.factories**: file with which to define the configuration classes when an application includes this library.
...
With this file we define the configuration classes to be loaded when the library is included in an application. In the example above the content of this file would be as follows:

    org.springframework.boot.autoconfigure.EnableAutoConfiguration=com.santander.darwin.greetings.config.HelloWorldAutoConfig

## Library use cases

Once we have created the Spring Boot project with the library, we are going to show a use case in an application.
For this example,
we will rely on a Web application
generated with the [Darwin Spring Boot microservice archetype](../darwin-spring-boot-archetype-microservice/README.md).

### Install artifact in maven repository

In order to be able to add the dependency with the new library later in the application, it must be in some maven repository with which it has visibility. For this example we will use our team's local repository, running the `install` phase of the
...
With the artefact installed in the repository, we can now add the library dependency to the application's POM along with the rest of the dependencies:
...
Thanks to the `spring.factories` file, when we start the application our library will be configured automatically. To test it, we have included the following changes in the application:
