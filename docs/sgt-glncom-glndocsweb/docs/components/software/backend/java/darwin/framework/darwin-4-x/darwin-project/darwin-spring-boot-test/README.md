# Darwin Spring Boot Test ![4.3.3-RELEASE](https://img.shields.io/badge/4.3.3-RELEASE-FF073D)

![GA](https://img.shields.io/badge/GA-C81D11)

## Description

The `Darwin Spring Boot Test` library provides projects with a series of "*Out Of The Box*" components and functionalities that **facilitate the design and implementation of an application's test cases**. These **extend, simulate or simplify the
normal behavior of some test components and Darwin** under certain execution conditions: Logging in test mode, *mocks* for Common Services (PKM, STS …​), etc.

## Functionality

### Console log traces

If the project includes the starter of the [Darwin Logging Library](../darwin-spring-boot-logging/README.md) among its dependencies, the following will be configured:
...
If the project includes the starter of [Darwin Metrics Library](https://gluon.dev.corp/microservices/docs/darwin-spring-boot/4.3.3-RELEASE/darwin-project/darwin-spring-boot-metrics/) among its dependencies,
...
This functionality by default is disabled, being able to enable them from the [configuration property](#configuration-properties) `darwin.test.logging.metrics-console`

### Log Tester and LoggingServiceConfig

If the project includes the starter of [Darwin Logging Library](../darwin-spring-boot-logging/README.md) among its dependencies, it will be possible to configure a test *appender* in order to capture and check the traces sent
through it.

This will allow us to check whether the traces are sent and have the correct and proper format. With this, we will test the log reliant components of the architecture. In order to be able to use these test utils, we will need to extend the class
**LoggingServiceConfig** in our test class and add to the configuration the **TestLog4j2Appender** class.

### PKM and STS services mockup

If the project includes the [Authentication Library](../darwin-spring-boot-security-authentication/README.md) starter among its dependencies, the *Beans* in charge of invoking the **PKM and STS services will be replaced by other
counterparts but with this mocked functionality** . In this way **invocations and dependencies with external services are avoided during the execution of the tests**, without losing the validation and token conversion functionalities.

!!! note

    The **CircuitBreaker and Retry** implementations for communications with the PKM and STS will not be necessary, since the mocks will always return the configured information.

It is possible to disable these mocks from the [configuration properties](#configuration-properties) of Test, or all the Darwin security from [configuration
properties](../darwin-spring-boot-security-authentication/README.md#configuration) of Authentication.

!!! info "Important"

    Despite disabling test mocks or Darwin's security, **Spring's default security will still be active**.
...
!!! note
    ...
    Below are some examples to modify the configuration [of the PKM](#overriding-default-pkm-mock-values) and [of the STS](#overriding-the-default-values-of-the-sts-mock).

### Operational Control services mocks

If the project includes the starter of the [Authorization Library](../darwin-spring-boot-security-authentication/README.md) among its dependencies, the *Beans* in charge of invoking Darwin's **CO (Operational Control) services
...
It is possible to disable these mocks from the [configuration properties](#configuration-properties) of Test, disabling Darwin security from [configuration properties](../darwin-spring-boot-security-authentication/README.md#configuration)
of Authentication or disabling the [Authentication mocks for Test](#pkm-and-sts-services-mockup).

!!! info "Important"

    Despite disabling test mocks or Darwin security, **Spring's default security will remain active**.
...
If you have configured the **Multi-Entity Operational Control**, the *Bean* in charge of invoking this service will be replaced by one with this mocked functionality, and not those of the Darwin Operational Control.
...
It is possible to disable these mocks from the [configuration properties](#configuration-properties) of Test with the property `darwin.test.code-tables.enabled`.
...
|---------------------------------------------|-----------------------------------|-------|--------------------------------------------------------------------------------------------------------|---------------------------------------|
| darwin.test.logging.functional-console      | true                              | No    | Enable the output of the Functional log by console                                                     | boolean                               |
| darwin.test.logging.security-console        | true                              | No    | Enable the Security log output by console                                                              | boolean                               |
| darwin.test.logging.technical-pattern       | *CONSOLE*                         | No    | Pattern for the Technical log by console                                                               | TechnicalPattern (*CONSOLE* o *JSON*) |
| darwin.test.logging.metrics-console         | false                             | No    | Enable metrics output by console                                                                       | boolean                               |
| darwin.test.security.enabled                | true                              | No    | Enable the configuration of mocks for Common Services (PKM, STS, etc.)                                 | boolean                               |
| darwin.test.security.private-key            | (Private key of a valid key pair) | No    | Private key to use to generate the JWT token from the TokenUtils class                                 | String                                |
| darwin.test.security.public-key             | (Public key of a valid key pair)  | No    | Public key with which to validate the token generated with the TokenUtils class                        | String                                |
| darwin.test.security.issuer                 | darwin                            | No    | *Issuer* for the *ClaimSet* of the JWT token generated with the TokenUtils class                       | String                                |
| darwin.test.security.audience               | \[spain\]                         | No    | *Audience* for the *ClaimSet* of the JWT token generated with the TokenUtils class                     | Set&lt;String&gt;                         |
| darwin.test.security.user.consultive        | defaultUser                       | No    | User/*Subject* for query JWT token generated with TokenUtils class                                     | String                                |
| darwin.test.security.user.operative         | OperUser                          | No    | User/*Subject* for the operational JWT token generated with the TokenUtils class                       | String                                |
| darwin.test.security.user.codConsultive     | F123456789                        | No    | ClientId that must be included in the parameters of a query type request with client validation        | String                                |
| darwin.test.security.user.codOperative      | F123456789                        | No    | ClientId that must be included in the parameters of an operational type request with client validation | String                                |
| darwin.test.security.user.contractId        | 004900010010000001                | No    | ContractId that must be included in the parameters of a request with contract validation               | String                                |
| darwin.test.security.authorization. enabled | true                              | No    | Enable mock configuration for CO services                                                              | boolean                               |
| darwin.test.code-tables.enabled             | true                              | No    | Enable mock configuration for CodeTables service                                                       | boolean                               |
...
| MockPKMService             | PKMService              | Simulates the invocation to the PKM service returning the configured public key (must be valid against the private key also configured) | <ul><li>All</li></ul> |
...
| MockCOCConnector           | COCConnector            | Simulates the invocation of Darwin's Channel Operational Control                                                                        | <ul><li>All</li></ul> |
| MockCONConnector           | CONConnector            | Simulates the invocation of Darwin's Business Operational Control                                                                       | <ul><li>All</li></ul> |
...
!!! note
    ...
    The [Darwin Authorization rules](../darwin-spring-boot-security-authorization/README.md#darwin-operative-security) must continue to be complied with according to the client parameters and the operation contract.
...
- **Scope Business**. For operations that have Business scope, an attempt is made to retrieve the Joc token channel if it already exists in context, otherwise the Darwin channel will be used:
...
!!! note
    ...
    The [Multi-entity Authorization rules](../darwin-spring-boot-security-authorization/README.md#multi-entity-operational-security) must continue to be complied with according to the client parameters and the operation
    contract.
...
The answers we can get with the original [Darwin CodeTables service](https://gluon.dev.corp/microservices/docs/darwin-spring-boot/4.3.3-RELEASE/apidocs/com/santander/darwin/codetables/service/package-summary.html) are:
...
you must also take into account the model [CodeTable](https://gluon.dev.corp/microservices/docs/darwin-spring-boot/4.3.3-RELEASE/apidocs/com/santander/darwin/codetables/domain/CodeTable.html)
that uses the [Darwin CodeTables library](https://gluon.dev.corp/microservices/docs/darwin-spring-boot/4.3.3-RELEASE/darwin-project/darwin-spring-boot-codetables/):
     ...
     @BeforeAll
        void create() {
            ...
            MockEnvironment environment = new MockEnvironment()
                    .withProperty("logging.level.com.santander.darwin.logging.logger", "INFO");

            DefaultLoggingConfigBuilder builder = (DefaultLoggingConfigBuilder) LoggingConfig.builder(() -> false);
            builder.setEnvironment(environment);

            new DarwinLoggingConfiguration(builder, properties, environment);
...
#### Overriding default PKM mock values
...
It is possible to configure another token from the `darwin.test.security.bks-token` property of the library:
