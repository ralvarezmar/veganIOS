...
- Fixed bug relating to .gitignore. From now on, .gitignore is properly located in the microservice just created.Furthermore, some errors have been corrected in the README file of the generated project.
...
- Swagger 2 usage is changed to Springdoc OpenAPI. The new configuration is added to the archetype so that, when creating a new application, the API's documentation generator is fully operational. Only applies to web environments.
