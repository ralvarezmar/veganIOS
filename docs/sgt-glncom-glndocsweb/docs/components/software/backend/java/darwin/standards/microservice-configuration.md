...
## Darwin Spring Boot microservices
...
!!! Note
    ...
    You could start a Darwin Spring Boot microservice from test scope using the provided `ApplicationTestRun` class.
...
- The architecture properties will have the root of darwn. E.g. darwin.logging.
- The application properties will have the root app. E.g. app.prop1
- Properties whose value contains an internal path to another microservice must add a suffix to the service name. Example app.serviciox=<http://serviciox${DARWIN_SUFFIX}:8080>
...
app:
    cod-producto: 1212
    ruta-cuentas: http://cuentas${DARWIN_SUFFIX}:8080
```
