...
- Improve Operative Control authorization to support path and query parameters in Post validations.
...
- Updated documentation for @OperativeControl annotation which states that now is not available in @Async modes.
...
- Fix issue using using Multi-entity operational security process with CON in white-list and business validation. Now we don't check business in this case
...
- Update Darwin authorization rules to **call Business Control Operation when client validation is false too**.
...
- Fix the way that context was progressed to authorization services in Servlet applications with `darwin.core.reactor.context-propagation=AUTO` property.
...
- Adding a new authorization manual mode:
  Portugal manual authorization.
...
- Adding the `darwin.security.authorization.manual.mode` property o expose the authorization service component
  for applying the operative security manually and to disable the annotation instrumentation in web applications.
...
- Configuration property has been created to define which authorization processes should be loaded.
  Also, a configuration property has been created to set a priority over the available authorization processes.
