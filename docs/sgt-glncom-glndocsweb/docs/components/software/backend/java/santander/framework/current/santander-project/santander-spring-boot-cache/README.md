# Santander Spring Boot Cache ![1.2.1](https://img.shields.io/badge/1.2.1-FF073D)

![Current](https://img.shields.io/badge/CURRENT-30FF24) ![GA](https://img.shields.io/badge/GA-C81D11)

## Description

Library in charge of managing the santander-based microservices cache. It currently has support for Caffeine and Data Grid.
...
|------------------------------------------------------------------------------------------|-----------------------------------------------------------------|-----------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------|
| santander.cache.automatic-mode.enabled                                                      | false                                                           | No        | Allows a microservice to simultaneously cache both locally with *Caffeine* and in a distributed *Data Grid*                                                                                | Boolean                                  |
| santander.cache.automatic-mode.cache-type-by-priority                                       | [INFINISPAN,CAFFEINE]                                           | No        | Indicates in which order the automatic cache will search the cache. For example, with the default values [INFINISPAN,CAFFEINE] it goes first to INFINISPAN (Datagrid) and then to CAFFEINE | List&lt;CacheType&gt;                    |
| santander.cache.automatic-mode.cache-name-by-type                                           | null                                                            | No        | By default the automatic cache prompts cache managers to check which cache names they contain. Using this map, you can indicate which cache names will be looked for in each cache type    | Map&lt;CacheType, List&lt;String&gt;&gt; |
| santander.cache.circuitbreaker.enabled                                                      | true                                                            | No        | Add a Circuit Breaker to cache calls                                                                                                                                                       | Boolean                                  |
| santander.cache.infinispan.use-auth                                                         | true                                                            | No        | Indicates whether to use authentication on the connection. In the case that it is disabled, the actuator will not be able to be used with the cache statistics                             | Boolean                                  |
| santander.cache.infinispan.auth-username                                                    | null                                                            | No        | Cache connection user                                                                                                                                                                      | String                                   |
| santander.cache.infinispan.auth-password                                                    | null                                                            | No        | Password for cache connection user                                                                                                                                                         | String                                   |
| santander.cache.infinispan.auth-realm [see](README.md#authentication-mechanisms)            | ApplicationRealm                                                | No        | Application realm against which we authenticate                                                                                                                                            | String                                   |
| santander.cache.infinispan.auth-server-name [see](README.md#authentication-mechanisms)      | jdg-server                                                      | No        | Server name against which we authenticate                                                                                                                                                  | String                                   |
| santander.cache.infinispan.sasl-mechanism [see](README.md#authentication-mechanisms)        | DIGEST-MD5                                                      | No        | Algorithm used for authentication verification                                                                                                                                             | String                                   |
| santander.cache.infinispan.read-timeout                                                     | 200                                                             | No        |                                                                                                                                                                                            | Long                                     |
| santander.cache.infinispan.write-timeout                                                    | 200                                                             | No        |                                                                                                                                                                                            | Long                                     |
| santander.cache.caffeine.async [see](README.md#cachemono-with-automatic-mode-enabled) | true                                                            | No        | Allows Caffeine caches to be created in asynchronous mode by default. Necessary for the use of cache annotations in reactive mode with Caffeine                                            | Boolean                                  |
| santander.cache.caffeine.allow-null-values                                                  | true                                                            | No        | Allows null values to be stored in the cache. It's recommended to set it to false for performance reasons                                                                                  | Boolean                                  |
...
| infinispan.remote.reactive [see](README.md#cachemono-with-automatic-mode-enabled)  | true                                                            | No        | Add support to use Datagrid caches in reactive mode. Necessary to use cache annotations in reactive microservices                                                                          | Boolean                                  |
...
!!! warning
    ...
    The **infinispan.remote.socket-timeout**, **infinispan.remote.connect-timeout** and **infinispan.remote.max-retries** properties are strongly recommended that they not be modified
...
So, for backwards compatibility, the *Santander Spring Boot framework sets this Java serialization marshaller as the default*.
...
If you want to use `JavaSerializationMarshaller` an example configuration is shown below:
    ...
    infinispan:
      remote:
        java-serial-allowlist: com.santander.framework.springboot.authentication.*
        marshaller: org.infinispan.commons.marshall.JavaSerializationMarshaller

#### Authentication mechanisms

To maintain compatibility with DataGrid 7.x versions, the `santander.cache.infinispan.auth-realm`, `santander.cache.infinispan.auth-server-name` and `santander.cache.infinispan.sasl-mechanism` properties are set to the values shown in this table.
...
When the Santander Spring Boot version used includes Infinispan `9.4.24.Final-redhat-00002`, there are a few things to be aware of:
...
If you want to use `santander-spring-boot-starter-cache-infinispan` in native mode, you also have to add the next dependencies:
...
More information on how to use these annotations can be found in [cache-annotations](https://docs.spring.io/spring-framework/docs/6.2.11/reference/html/integration.html#cache-annotations)
...
More information on [Spring Boot - Caffeine](https://docs.spring.io/spring-boot/docs/3.5.6/reference/html/io.html#io.caching.provider.caffeine) and more detailed property specifications in
[CaffeineSpec](https://www.javadoc.io/doc/com.github.ben-manes.caffeine/caffeine/3.2.2/com/github/benmanes/caffeine/cache/CaffeineSpec.html)
...
[Caffeine](https://www.javadoc.io/doc/com.github.ben-manes.caffeine/caffeine/3.2.2/com/github/benmanes/caffeine/cache/Caffeine.html).
...
The automatic mode allows you to use several cache managers at the same time. To use it, the *santander.cache.automatic-mode.enabled* property must be enabled.
...
!!! note
    ...
    For both cache managers to be in the dependencies it is necessary to load both cache starters: **santander-spring-boot-starter-cache-caffeine**, **santander-spring-boot-starter-cache-infinispan**

The default configuration sets the **santander.cache.automatic-mode.cache-type-by-priority** property to **\[INFINISPAN, CAFFEINE\]**. With this configuration the operation is as follows: In the first request to the cache, the microservice will ask
...
For each cache type, you can decide whether the name of the caches to be managed is obtained from the cache manager itself (default behavior) or configured manually using the **santander.cache.automatic-mode.cache-name-by-type.\[CACHE-TYPE\]**
property.
...
the cache list manually in the property **santander.cache.automatic-mode.cache-name-by-type.INFINISPAN** and not to indicate the **cache1** cache.
...
the last cache manager defined in **santander.cache.automatic-mode.cache-type-by-priority**, by default **Caffeine**,
...
Since Santander Spring Boot 5.0.x you can use cache annotations in reactive microservices. This is the way we
we recommend to use the cache. However, you can also use the `CacheMono` bean provided by Santander Spring Boot.

If we are using **Caffeine**, it is necessary that the caches created are of the asynchronous type. By default, the
caches are created of synchronous type. That is why we have added the following property `santander.cache.caffeine.async'.
...
The santanderCacheMono service is available for use. Example of its use:

    @Autowired
    private SantanderCacheMono santanderCacheMono;
    ...
    santanderCacheMono.cache("cacheName","cacheKey", monoObjectToBeCached)
...
#### CacheMono with automatic mode enabled

If the automatic mode is active, SantanderCacheMono in the first call to the *cache* method will get from Data Grid the list of caches it has defined. From there,
if the *cacheName* exists in Data Grid it will be cached in Spring and otherwise it will be cached in Caffeine.
