# Santander Spring Boot Partenon ![1.2.1](https://img.shields.io/badge/1.2.1-FF073D)

![Current](https://img.shields.io/badge/CURRENT-30FF24) ![GA](https://img.shields.io/badge/GA-C81D11)

## Description

The Partenon library extends [Spring Boot Partenon Connector](https://santandernet.sharepoint.com/sites/SEPBKSCCA/SitePages/Annexes/Documentation/Spring%20Libraries%20Documentation/Partenon%20TrxOp%20Spring%20Connector%20-%20User%20Manual.aspx),
...
The `Santander Spring Boot Partenon` library provides the following functionalities depending on the application type:

### Servlet Applications

The following is added to all the functionality of the Partenon Connector Spring Boot library:

#### Connectors associated with a channel
...
- A *Bean* of the `PartenonConnectSwitch` class is exposed, which implements the\` PartenonConnectOperations\` interface (replacing the one exposed by the original library). This *Bean* will use the Connector corresponding to the channel found in
    the `Context` (or the default connector) to execute the transaction in a totally transparent way to the project.
...
- Before carrying out any transaction, the following headers of the PartenonAction object are initialized with default values **if they were not already defined** in it:
    ...
    - **LOGICALCHANNEL**: the value of the **logical channel** corresponding to the ContactPoint of the Context is assigned.

    - **PHYSICALCHANNEL**: the value of the **physical channel** corresponding to the ContactPoint of the Context is assigned if the property *physicalChannel* had not been configured for the connector.
...
- It exposes a *Bean* of the `PartenonConnectSwitchReactive` class, which implements the\` PartenonConnectOperationsReactive\` interface with functionality fully equivalent to **PartenonConnectSwitch**. This *Bean* will use the Connector
    corresponding to the channel found in the `Context` (or the default connector) to execute the transaction in a completely transparent way to the project.
...
|---------------------------|---------------|-------|-----------------------------------------------------------------------------|------------------------------------------------------------------------|
| santander.partenon           | N/A           | No    | Map with possible channels and their corresponding connections to Partenon. | Mapa                                                                   |
| santander.partenon.{channel} | N/A           | No    | Properties of the Partenon connector associated with that **channel**.      | [PartenonConnectProperties](https://santandernet.sharepoint.com/sites/SEPBKSCCA/SitePages/Annexes/Documentation/Spring%20Libraries%20Documentation/Partenon%20TrxOp%20Spring%20Connector%20-%20User%20Manual.aspx#3.2-application-properties) |
| santander.partenon.default   | N/A           | No    | Default Partenon connector properties.                                      | [PartenonConnectProperties](https://santandernet.sharepoint.com/sites/SEPBKSCCA/SitePages/Annexes/Documentation/Spring%20Libraries%20Documentation/Partenon%20TrxOp%20Spring%20Connector%20-%20User%20Manual.aspx#3.2-application-properties) |
...
### Santander Spring Boot Partenon property settings

The minimum that can be configured is a default connector that will be applied regardless of the channel, under the `santander.partenon.default` property:
...
!!! info "Important"
    ...
    In the Santander Spring Boot Partenon properties you must define **at least** the `host` and the\` port\` **of each connector**.
...
!!! info "Important"
    ...
    If the properties of Santander Spring Boot Partenon and the original Partenon library are defined, **only those of Santander Spring Boot Partenon** will be used to configure the connectors.

!!! tip "Caution"

    This setting may be deprecated in future versions of the Santander Spring Boot framework, so **we recommend using Santander Spring Boot** properties instead.
...
Added native compilation support for *Partenon* library in the Santander Spring Boot framework.
...
|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------|----------------------------------------------------------------------------------------------------------------------------|-------------------------------------------|
| [PartenonConnectSwitch](https://gluon.dev.corp/microservices/docs/santander-spring-boot/1.2.1/apidocs/com/santander/framework/springboot/partenon/connector/PartenonConnectSwitch.html)                 | PartenonConnectOperations         | Component that will act as a "switch" to use the Partenon Connector corresponding to the channel at that moment.           | <ul><li>NotWeb</li><li>Servlet</li></ul>  |
| [PartenonConnectSwitchReactive](https://gluon.dev.corp/microservices/docs/santander-spring-boot/1.2.1/apidocs/com/santander/framework/springboot/partenon/connector/PartenonConnectSwitchReactive.html) | PartenonConnectOperationsReactive | Reactive adaptation that will act as a "switch" to use the Partenon Connector corresponding to the channel at that moment. | <ul><li>NotWeb</li><li>Reactive</li></ul> |

## Santander Spring Boot - Partenon Connector

The Santander Spring Boot Partenon module extends the functionality provided by the original Partenon Connector, in this section we will explain in more detail the integration that has been done and the behavior of the connector for certain cases.

### Integration model

The **integration model between Santander Spring Boot's library and the Partenon Connector** is as follows:

![Santander Spring Boot Partenon Connector integration](../../images/Partenon-Servlet.png)

As we can see, Santander Spring Boot's `PartenonConnectSwitch` component wraps the Partenon Connector (with all the pieces that compose it: PartenonConnectTemplate, TrxOpServiceImpl) adding the [functionalities indicated above](#functionality):
add headers by default, be able to configure several connectors depending on the ContactPoint channel, etc.
...
![Santander Spring Boot Reactive Partenon Connector integration](../../images/Partenon-Reactive.png)
...
!!! info "Important"
    ...
    All the functionality explained below is specific to the Partenon Connector. Santander Spring Boot integration manages to extend this functionality without affecting the original.
...
!!! info "Important"
    ...
    The PartenonSecurityUtil class **is not prepared to retrieve the credentials from the reactive context**; however, Santander Spring Boot's `PartenonConnectSwitchReactive` class manages to replicate this functionality.
...
The [Gluon Java Partenon Maven-Plugin](https://gluon.gs.corp/community/docs/latest/components/software/backend/microservice/santander/framework/java-springboot/plugins/partenon-plugin/)
...
to call Partenon transactions in a Santander Spring Boot microservice.
...
Santander Spring Boot's library will have replaced the original *Bean* with the `PartenonConnectSwitch` implementation, but we can still use any of the original Partenon Connector methods:
...
!!! note
    ...
    Complete examples of transactions with the definition of classes and the use of these methods can be found in [Santander Spring Boot Samples:
    Partenon](https://github.com/santander-group-shared-assets/gln-back-santander-java-samples/tree/develop/partenon)
...
In Web Reactive applications we must inject a *Bean* from the `PartenonConnectOperationsReactive` interface:
    ...
    import com.santander.framework.springboot.partenon.connector.PartenonConnectOperationsReactive;
    ...
...
It should be noted that in this type of application **we do not have the filters that are responsible for filling in the Santander Spring Boot contexts** (with the ContactPoint) and Security (with the authenticated user),
necessary for certain functionalities that are offered.
...
To add to the **Context** the **ContactPoint** object corresponding to the desired channel, we must follow the same steps defined in this [Omnichannel library
example](https://gluon.dev.corp/microservices/docs/santander-spring-boot/1.2.1/santander-project/santander-spring-boot-omnichannel/), and once the `ContextHolder` finish executing the transaction:

    ...
    ContextHolder.getCurrentContext().setContactPoint(cp);

    TrxBPG8 inputTrx = new TrxBPG8("0049", "001", "001");

    // Now you can retrieve the ContactPoint from the ContextHolder
    TrxBPG8Result result = partenonConnect.executeTrx(inputTrx, TrxBPG8Result.class);

To now add an **Authentication** object with the user to the **SecurityContext**,
follow the steps defined in this [Security library example](../santander-spring-boot-security-authentication/README.md#how-to-use-security-services-in-notweb-applications) using
...
To add to the **Context** the **ContactPoint** object corresponding to the desired channel,
we must follow the same steps defined [example of the Omnichannel library](https://gluon.dev.corp/microservices/docs/santander-spring-boot/1.2.1/santander-project/santander-spring-boot-omnichannel/#how-to-retrieve-the-channel-from-the-context),
to finish executing the transaction by adding to the Context the Context with `ReactiveContextHolder`:

    ...
    Mono<Context> contextMono = Mono.just(context);
    ...
    partenonConnectReactive.executeTrx(inputTrx, TrxBPG8Result.class)
            .map(result -> ...)
            .suscriberContext(ReactiveContextHolder.withContext(contextMono));

To now add an **Authentication** object with the user to the **SecurityContext**, follow the steps defined in this [Security library example](../santander-spring-boot-security-authentication/README.md#how-to-use-security-services-in-notweb-applications)
...
- LOGICALCHANNEL: value of the logical channel corresponding to the ContactPoint of the Context.

- PHYSICALCHANNEL: value of the physical channel corresponding to the ContactPoint of the Context.
