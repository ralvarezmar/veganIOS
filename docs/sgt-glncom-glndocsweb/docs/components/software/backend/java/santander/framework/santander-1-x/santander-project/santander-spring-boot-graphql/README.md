# Santander Spring Boot GraphQL ![1.2.1](https://img.shields.io/badge/1.2.1-FF073D)

![Current](https://img.shields.io/badge/CURRENT-30FF24) ![GA](https://img.shields.io/badge/GA-C81D11)

## Description

The `Santander Spring Boot GraphQL` module provides support for Santander Spring Boot applications built
on **[Spring GraphQL](https://docs.spring.io/spring-graphql/docs/1.4.2/reference/html) and [GraphQL Java](https://www.graphql-java.com/)**. Includes Santander Spring Boot
Context support, Santander Spring Boot GraphQl error format, etc.

## Functionality

[Spring GraphQL](https://docs.spring.io/spring-graphql/docs/1.4.2/reference/html) integrates the [GraphQL Java](https://www.graphql-java.com/) library in a Spring Boot application including functionalities like: exposing the **GraphQl service as an
endpoint**, create and configure **mappings to resolve GraphQl operations** using annotations, etc.

!!! note

    You can find examples of Spring GraphQL functionalities in the [GraphQL project of Santander Spring Boot Samples](https://github.com/santander-group-shared-assets/gln-back-santander-java-samples/tree/develop/graphql).

The `Santander Spring Boot GraphQL` module extends the Spring GraphQL library providing support for Servlet/Reactive Santander Spring Boot applications with these functionalities:

- Ensures **Context access through ThreadLocal in GraphQL DataFetchers** when the execution thread is not the same that Spring MVC handler thread. This is only needed for Servlet applications.
...
!!! info "Important"
    ...
    Any error thrown out of the Spring GraphQL execution (before DispatcherServlet handling) is going to be resolved with the default Santander Spring Boot error management.
...
- Through `Authorization` library, the operative security can be applied by each GraphQL operation defined in the [Schema](https://www.graphql-java.com/documentation/schema).
  ...
  All the information about how to use `@OperativeControl` directive can be found in this [guide](../santander-spring-boot-security-authorization/README.md#how-to-use-operativecontrol-directive-on-graphql-schema).
...
!!! tip "Caution"
    ...
    If your application comes from a **previous Santander Spring Boot Spring Framework version**, you'll have to configure these lines in your **pom.xml** to include your GraphQL external fields in compile-time.
...
|-----------------------------------|---------------|-------|-------------------------------------------------------------------------------------|------------------|
| santander.graphql.exceptions.enabled | true          | No    | Boolean to enable/disable GraphQL errors resolution with the configured ErrorModel. | boolean          |
| santander.graphql.query-cache-size   | 100           | No    | Max number of queries to store in pre-parsed query cache.                           | short            |
...
|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------|--------------------------------------------------------------------------|------------------------------|
| [GraphQLErrorModel](https://gluon.dev.corp/microservices/docs/santander-spring-boot/1.2.1/apidocs/com/santander/framework/springboot/core/exceptions/dto/GraphQLErrorModel.html) | Interface | Interface to be implemented by ErrorModel POJOs to support GraphQLError. | <ul><li><p>All</p></li></ul> |
...
!!! info "Important"
    ...
    ***data*** field if there is data retrieved and/or ***errors*** field if there was any error during execution. For any other status code, the response body will be resolved by [Santander Spring Boot REST error
    management](../santander-spring-boot-core/README.md#exception-handling) (not GraphQL response format).
...
Santander Spring Boot GraphQL includes a resolver that allows to **set the *message* and *extensions* fields based on the ErrorModel configured in the application** if this implements the `GraphQLErrorModel` interface. The [ErrorModel
customization](../santander-spring-boot-core/README.md#customize-error-model-fields) will be also executed before getting the info from ErrorModel for GraphQL error.

!!! note

    *Locations* and *path* fields are set automatically by the Santander Spring Boot resolver.

!!! tip "Caution"

    Disabling [Santander Spring Boot Core Exception Management](../santander-spring-boot-core/README.md#exception-types) also disables this Santander Spring Boot GraphQL resolver.

### Santander Spring Boot GraphQLErrorModel

[Support](../santander-spring-boot-core/README.md#error-model) GraphQL error responses in addition to regular REST responses following this relation between both formats:
...
[Support](https://gluon.dev.corp/microservices/docs/santander-spring-boot/1.2.1/santander-project/santander-spring-boot-extended-error/)
...
You can find use cases including the main Spring GraphQl features (simple Spring GraphQl app, pagination, batch processing, QueryDSL) in the [Santander Spring Boot Samples
project](https://github.com/santander-group-shared-assets/gln-back-santander-java-samples/tree/develop/graphql).

### Adapt an ErrorModel for GraphQl error responses

An [ErrorModel configured for Santander Spring Boot Framework](../santander-spring-boot-core/README.md#replace-error-model) can be adapted easily to be used for GraphQl error responses. If this is the configured model:
