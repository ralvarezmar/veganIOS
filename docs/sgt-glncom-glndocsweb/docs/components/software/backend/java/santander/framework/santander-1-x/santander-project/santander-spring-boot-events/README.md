# Santander Spring Boot Events ![1.2.1](https://img.shields.io/badge/1.2.1-FF073D)

![Current](https://img.shields.io/badge/CURRENT-30FF24) ![GA](https://img.shields.io/badge/GA-C81D11)

## Description

The `Santander Spring Boot Events` library provides the necessary functionality to generate and/or consume events from `Kafka` topics.
...
!!! info "Important"
    ...
    If you need information about the `santander-spring-boot-business-events` library, please refer to the [Business Events](../santander-spring-boot-business-events/README.md) documentation.

A `Builder` named `CloudEventBuilder` is provided to generate `CloudEvent` events. The `CloudEvent` type events
will be events formed from a composition of the original `CloudEventBuilder` plus a `CloudEventExtension` named
`Extension` which allows us to inject `Santander Spring Boot` own parameters, therefore the `CloudEvent` type events will be compatible with the `CloudEvent` specification.
...
An object of type `CloudEvent` is decomposed into a set of `Kafka Headers + Body` that are sent to the topic. The following describes the information that is sent/retrieved when a `CloudEvent` message is sent/received.
...
| ce_channel            | String         | Optional  | Santander Spring Boot      | Identifies the channel through which the user interacts.                                                                                                                                                    | "INT"                                                                                       |
| ce_sessionid          | String         | Optional  | Santander Spring Boot      | Identifies the user's session in the application.                                                                                                                                                           | "7fddae8e-8977-11e0-bc11-003048c3b1f2"                                                      |
| ce_acceptlanguage     | String         | Optional  | Santander Spring Boot      | Information about internationalization.                                                                                                                                                                     | "es_ES"                                                                                     |
| ce_entity             | Integer        | Optional  | Santander Spring Boot      | International bank identifier (BIC, code 8, ISO 9632).                                                                                                                                                      | "0049"                                                                                      |
| ce_authorization      | String         | Optional  | Santander Spring Boot      | Authentication token associated with the user.                                                                                                                                                              | "TWFuIGlzIGRpc3Rpbmd1aXNoZWQsIG5vd…​"                                                       |
| ce_operationalcontrol | String         | Optional  | Santander Spring Boot      | Token with the operational control validation data.                                                                                                                                                         | "gYSBwZXJzZZlcmFuY2Ugbm5hbCBwbGV2Yg…​"                                                      |
...
To create a `CloudEvent` from the `Builder` we can use the specific methods for each attribute, use the `Santander Spring Boot` contexts or even some of them will be automatically filled with default values.
The following table shows how each of the `CloudEvent` fields are populated:
...
| ce_channel            | It can be initialized in two ways, through `CloudEventBuilder().withChannel("value")` or automatically from the information stored in the `Info` object located in `Context`; for the latter to work it is necessary to have `set` the context through `CloudEventBuilder().withInfo(info)` . In case of not initializing it, the default value will be `null`.                                                                                       |
| ce_sessionid          | It can be initialized in two ways, by `CloudEventBuilder().withSessionId("value")` or automatically from the information stored in the `Info` object located in `Context`; for the latter to work it is necessary to have `set` the context by `CloudEventBuilder().withInfo(info)`. If not initialized, the default value will be `null`.                                                                                                            |
| ce_acceptlanguage     | It can be initialized in two ways, by `CloudEventBuilder().withAcceptLanguage("value")` or automatically from the information stored in the `Info` object located in `Context`; for the latter to work it is necessary to have `set` the context by `CloudEventBuilder().withInfo(info)`. It does not apply a default value, since being a mandatory field it must always be initialized.                                                             |
| ce_entity             | It can be initialized in two ways, by `CloudEventBuilder().withEntity(value)` or automatically from the information stored in the `Info` object located in `Context`; for the latter to work it is necessary to have `set` the context by `CloudEventBuilder().withInfo(info)`. It does not apply a default value, since being a mandatory field it must always be initialized.                                                                       |
| ce_authorization      | It can be initialized in two ways, by `CloudEventBuilder().withAuthorization("value")` or automatically from the information stored in the `Authentication` object located in the security context; for the latter to work it is necessary to have `set` the context by `CloudEventBuilder().withAuthentication(authentication)`. If not initialized, the default value will be `null`.                                                               |
| ce_operationalcontrol | It can be initialized in two ways, by `CloudEventBuilder().withOperationalControl("value")` or automatically from the information stored in the `Authentication` object located in the security context; for the latter to work it is necessary to have `set` the context by `CloudEventBuilder().withAuthentication(authentication)`. If not initialized, the default value will be `null`.                                                          |
...
- The `CloudEvent` extension named `Extension` is registered.
...
By default, Santander Spring Boot offers protection against poison pill scenarios. A poison pill can be a corrupted event or an event that produces a deserialization error.
...
Santander Spring Boot will use an ErrorHandlingDeserializer to detect this type of events and consume them so that the service is maintained passing to the next event.

To disable this capability just configure the property:

    santander.events.deserialization-protection-enabled: false
...
The event module auto-configures a `HealthIndicator` that allows to configure the [`Kubernetes probes`](https://docs.spring.io/spring-boot/docs/3.5.6/reference/html/actuator.html#production-ready-kubernetes-probes) depending on the connectivity with
Kafka.

This indicator makes use of [Spring Boot Actuator](https://docs.spring.io/spring-boot/docs/3.5.6/reference/html/actuator.html#production-ready) to add a new component under the health endpoint `http://localhost:8080/actuator/health/kafka`, whose
...
`kafka` cluster, since they are based on the [events that Spring Boot launches](https://docs.spring.io/spring-boot/docs/3.5.6/reference/html/#production-ready-kubernetes-probes-lifecycle) during the life cycle of the application, therefore, it is
...
### Automatic generation of Santander Spring Boot Session and security context for received events

The library allows the automatic generation of the Santander Spring Boot Session and security context for received events.
Santander Spring Boot uses the request headers to generate the Santander Spring Boot Session and security context.
It's only for non-reactive listeners.

This feature is enabled by default.
To disable this feature, set the property `santander.events.context-creation.enabled` to `false`.
...
!!! note
    ...
    All observability features are in the `santander-spring-boot-logging` library. Please refer to the `santander-spring-boot-logging` documentation for more information.

## Installation and configuration

To add the library in any project you will have to include the `starter` in your `pom.xml` file. **Santander Spring Boot Events**:
...
It is not necessary to include the `santander-spring-boot-starter-events` dependency to use `santander-spring-boot-starter-business-events` or the other way around.
...
#### General properties (`santander-spring-boot-starter-events`)

The properties in the attached table only apply if the `santander-spring-boot-starter-events` starter has been added to the application.

| Name                                             | Default value | Required | Description                                                                                                                                                  | Supported values                    |
|--------------------------------------------------|---------------|----------|--------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------|
| santander.events.enabled                            | true          | No       | Allows to disable the library autoconfiguration.                                                                                                             | Boolean                             |
| santander.events.format                             | AVRO          | Yes      | Allows to indicate the event format of CloudEvent.                                                                                                           | AVRO and/or JSON                    |
| santander.events.deserialization-protection-enabled | true          | No       | Enables and disables protection against deserialization errors.                                                                                              | Boolean                             |
| santander.events.resilience-mode                    | false         | No       | Enables to set the resilience mode.                                                                                                                          | `deliveredguarantee` or `realtime`. |
| santander.events.context-creation.enabled           | true          | No       | Enables and disables the automatic generation of the Santander Spring Boot Session and security context for received events.                                                | Boolean                             |
...
### Santander Spring Boot Events properties configuration
...
!!! info "Important"
    ...
    If `santander.events.format` is **custom**, By default the kafka producer will configure the value-serializer property set to `org.apache.kafka.common.serialization.StringSerializer` and the kafka consumer will have its
...
property. Santander Spring Boot sets an initial value of 60 seconds. In case it is necessary to set a custom time interval (seconds) for the `authorizationExceptionRetryInterval` property, the following configuration will be used:
...
To disable the automatic generation of activity traces associated with the consumption and sending of messages, set the `santander.logging.activity.enabled` property to `false`.
...
!!! note
    ...
    For more information, see the [configuration](../santander-spring-boot-logging/README.md#configuration) section of the `Logging` module.
...
|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------|-------------------------------------------------------------------------------------------------------------|------------------------------------------|
| [CloudEvent](https://gluon.dev.corp/microservices/docs/santander-spring-boot/1.2.1/apidocs/com/santander/framework/springboot/events/CloudEvent.html)                                  | CloudEvent                | Exposes a Builder that allows us to create events of type CloudEvent.                                 | <ul><li>All</li></ul>                    |
| [Extension](https://gluon.dev.corp/microservices/docs/santander-spring-boot/1.2.1/apidocs/com/santander/framework/springboot/events/extension/Extension.html)                          | Extension                       | Extension of CloudEvent for attributes specific to Santander Spring Boot and which are not part of the standard CloudEvent | <ul><li>All</li></ul>                    |
| [CloudEventAvroSerializer](https://gluon.dev.corp/microservices/docs/santander-spring-boot/1.2.1/apidocs/com/santander/framework/springboot/events/serializers/CloudEventAvroSerializer.html)      | Serializer&lt;CloudEvent&gt;    | Serializer that allows to serialize CloudEvent events in Avro format.                                       | <ul><li>All</li></ul>                    |
| [CloudEventAvroDeserializer](https://gluon.dev.corp/microservices/docs/santander-spring-boot/1.2.1/apidocs/com/santander/framework/springboot/events/serializers/CloudEventAvroDeserializer.html)  | Serializer&lt;CloudEvent&gt;    | Deserializer that allows to deserialize CloudEvent events in Avro format.                                   | <ul><li>All</li></ul>                    |
| [AvroCloudEventData](https://gluon.dev.corp/microservices/docs/santander-spring-boot/1.2.1/apidocs/com/santander/framework/springboot/events/data/AvroCloudEventData.html)                         | CloudEventData                  | Class that allows encapsulating Avro types as CloudEventData.                                               | <ul><li>All</li></ul>                    |
| [ReactiveContextUtil](https://gluon.dev.corp/microservices/docs/santander-spring-boot/1.2.1/apidocs/com/santander/framework/springboot/events/reactive/ReactiveContextUtil.html)                   | ReactiveContextUtil             | Bean that allows to work with Santander Spring Boot context and security context over a reactive stream in a simple way.   | <ul><li>Reactive</li></ul>               |
| [ReactiveKafkaConsumerFactory](https://gluon.dev.corp/microservices/docs/santander-spring-boot/1.2.1/apidocs/com/santander/framework/springboot/events/reactive/ReactiveKafkaConsumerFactory.html) | ConsumerFactory                 | Factory that allows the creation of reactive Kafka consumers with the default Santander Spring Boot configuration.         | <ul><li>All</li></ul>                    |
| [ReactiveKafkaProducerFactory](https://gluon.dev.corp/microservices/docs/santander-spring-boot/1.2.1/apidocs/com/santander/framework/springboot/events/reactive/ReactiveKafkaProducerFactory.html) | ProducerFactory                 | Factory that allows the creation of reactive Kafka producers with Santander Spring Boot's default configuration.           | <ul><li>All</li></ul>                    |
| [KafkaHealthIndicator](https://gluon.dev.corp/microservices/docs/santander-spring-boot/1.2.1/apidocs/com/santander/framework/springboot/events/actuate/KafkaHealthIndicator.html)                  | HealthIndicator                 | Class that defines the Health strategy for Kafka brokers.                                                   | <ul><li>All</li></ul>                    |

## Use cases

### How to send a CloudEvent to a Kafka topic

#### JSON format

To send a `CloudEvent` to a Kafka topic in `Json` format you don't have to do anything special, since the library autoconfigures everything necessary,
mainly the binary mode of transmission of `CloudEvents` and the class in charge of
...
!!! info "Important"
    ...
    The library enables `Avro` mode by default, so if you want to work in `Json` mode it is necessary to define the `santander.events.format=JSON` property.
...
###### Non-reactive environments
    ...
    public class KafkaApplicationCloudEvent {
        ...
        public ApplicationRunner runner(KafkaTemplate<Long, CloudEvent> template) { (2)
            ...
            ContextHolder.setCurrentContext(context); (3)
            CloudEvent cloudEvent = CloudEvent.builder() (4)
                            .withBaseCloudEvent(
                            ...
                            .withInfo(ContextHolder.getCurrentContext().getInfo())
                            .build();
            return args -> {
...
3. We create a `Context` object and initialize the context.
    ...
    public class KafkaApplicationCloudEvent {
        ...
        public ApplicationRunner runner(KafkaSender<Long, CloudEvent> kafkaSender, (2)
            ...
            Info info = Info.builder()
                                              ...
                                              .locale(new Locale("he"))
                                              .build();
            context.setInfo(info); (3)

            return args -> {
                kafkaSender.send(
                    ReactiveContextUtil.getMonoContext()
                        .flatMap(function(
                            (Context dContext, SecurityContext securityContext) ->
                                Mono.just(1).map(i -> {
                                    Info dInfo = dContext.getInfo();
                                    CloudEvent cloudEvent = CloudEvent.builder() (4)
                                        .withBaseCloudEvent(
                                            CloudEventBuilder.v1()
                                                 .withId("1")
                            ...
                            context ->
                                ReactiveContextUtil.create(context, context, null) (7)
                        )
                ).subscribe(); (8)
            };
        }
...
2. We inject the `Bean kafkaSender` provided by `Santander Spring Boot`. This `Bean` allows us to send messages to a `Kafka` topic by using reactive streams.

3. We create a `Context` object.
...
7. We subscribe the Reactor context for the generation of the Context and the SecurityContext. This creation should always be done manually to ensure that these will be available for initialization in the *upstream*.
...
###### Reactive environments
    ...
    import reactive.com.santander.framework.springboot.events.ReactiveContextUtil;
    import lombok.Data;
    import io.cloudevents.core.data.PojoCloudEventData;
                ...
                kafkaSender.send(
                    ReactiveContextUtil.getMonoContext()
                        .flatMap(function(
                            ...
                            context ->
                                ReactiveContextUtil.create(context, context, null)
                        )
                ).subscribe();
            };
        }
...
To send a `CloudEvent` to a kafka topic in `Avro` format we will require the use of an [`Avro Schema`](https://avro.apache.org/docs/1.11.1/specification)
associated to the information to be sent, this will imply the execution of a series of
...
!!! info "Important"
    ...
    The Santander Spring Boot archetype will include the plugin in the pom.xml of the project automatically otherwise it is necessary to add it, since Santander Spring Boot only provides the default configuration of the plugin.
...
##### Non-reactive environments
    ...
    import data.com.santander.framework.springboot.events.AvroCloudEventData;
    ...
    public class KafkaApplicationCloudEvent {
        ...
        public ApplicationRunner runner(KafkaTemplate<Long, CloudEvent> template) { (2)
            ...
            CloudEvent cloudEvent = CloudEvent.builder() (5)
                .withBaseCloudEvent(
                        CloudEventBuilder.v1()
                                 .withId("1")
                                 .withSource(URI.create("http://localhost/source"))
                ...
                kafkaSender.send(
                    ReactiveContextUtil.getMonoContext()
                        .flatMap(function(
                            ...
                            context ->
                                ReactiveContextUtil.create(context, context, null) (8)
                        )
                ).subscribe(); (9)
            };
        }
...
2. We inject the `Bean KafkaSender` provided by `Santander Spring Boot`. This `Bean` allows us to send messages to a `Kafka` topic.
...
8. We subscribe the Reactor context for the generation of the Context and the SecurityContext. This creation should always be done manually to ensure that these will be available for initialization in the *upstream*.
...
### How to receive a CloudEvent from a Kafka topic

As in sending, receiving and processing messages of type `CloudEvent` from a Kafka topic does not require any special configuration, since the library autoconfigures everything necessary,
mainly the class in charge of deserializing the message
...
### How to transform CloudEvent into Santander CloudEvent

The `CloudEvent` interface will give us access to all the attributes of the standard specification but to access Santander Spring Boot's own attributes we will need to make use of the `CloudEvent` type,
for example to retrieve the value of the entity, channel, etc.
...
For this we have two alternatives, the first one is based on the `getExtension` method provided by the `CloudEvent` specification:
    ...
    CloudEvent event = ...; (1)
    String channel = event.getExtension(Extension.CHANNEL); (2)
    Integer entity = event.getExtension(Extension.ENTITY);
    (...)

1. We start from an event of type `CloudEvent` created from somewhere, either the application or retrieved from a topic.

2. Using the `getExtension` method and `Extension.XXX` constants we retrieve the `Santander Spring Boot` eigenvalues.

The second one is based on the transformation of the event to an `POJO` of type `Extension`:

    CloudEvent event = ...; (1)
    Extension extension = ExtensionProvider.getInstance().parseExtension(Extension.class, event); (2)

    String channel = extension.getChannel(); (3)
    Integer entity = extension.getEntity();
    (...)

1. We start from an event of type `CloudEvent` created from somewhere, either the application or retrieved from a topic.

2. We transform the event from type `CloudEvent` to type `Extension`.

3. Using the methods provided by `Extension` we retrieve the `Santander Spring Boot` own values.
...
### How to create CloudEvents

#### Creating CloudEvent without contexts

If we do not have a `Santander Spring Boot` or security context, we can create a `CloudEvent` object from the methods provided by the `Builder` itself.

    import io.cloudevents.CloudEvent;
    import io.cloudevents.core.builder.CloudEventBuilder;
    import com.santander.framework.springboot.events.CloudEvent;
    import java.net.URI;
...
#### Creating CloudEvent with contexts

If we have a `Santander Spring Boot` or security context, we can create a `CloudEvent` object from them through the `Builder`.
...
###### Non-reactive environments
    ...
    import com.santander.framework.springboot.core.context.Context;
    import com.santander.framework.springboot.core.context.Info;
    import io.cloudevents.CloudEvent;
    import io.cloudevents.core.builder.CloudEventBuilder;
    import com.santander.framework.springboot.events.CloudEvent;
    import com.santander.framework.springboot.common.context.ContextHolder;
    import java.net.URI;
    import java.time.OffsetDateTime;
    import org.springframework.security.core.Authentication;
    import org.springframework.security.core.context.SecurityContextHolder;

    public class ClassicCloudEvent {

        public CloudEvent getCloudEvent() {
            Info info = ContextHolder.getCurrentContext().getInfo();
...
To facilitate access to the Santander Spring Boot context and the security context in reactive environments, and to have both in the business logic, the
[ReactiveContextUtil](https://gluon.dev.corp/microservices/docs/santander-spring-boot/1.2.1/apidocs/com/santander/framework/springboot/events/reactive/ReactiveContextUtil.html) class exposes two static methods that extract them,
...
###### Mono type reactive stream
    ...
    import com.santander.framework.springboot.events.CloudEvent;
    import com.santander.framework.springboot.core.context.Context;
    import com.santander.framework.springboot.events.reactive.ReactiveContextUtil;
    import org.springframework.security.core.context.SecurityContext;
    import reactor.core.publisher.Mono;

    import static reactor.function.TupleUtils.function;

    public class ReactiveCloudEvent {

        ReactiveContextUtil contextUtil;

        public ReactiveCloudEvent(ReactiveContextUtil contextUtil) {
            this.contextUtil = contextUtil;
        }

        public Mono<CloudEvent> getMonoCloudEvent() {
            return
                ReactiveContextUtil.getMonoContext() (1)
                    .map(function(
                        (Context context, SecurityContext securityContext) ->
                            CloudEvent.builder()
                                .withBaseCloudEvent(CloudEventBuilder.v1()
                                    .withId("id")
                                    .withSource(URI.create("http://localhost/source"))
...
###### Flux type reactive stream
    ...
    public class ReactiveCloudEvent {
        ...
        public Flux<CloudEvent> getFluxCloudEvent() {
            return
...
###### Non-reactive environments
    ...
    import io.cloudevents.core.builder.CloudEventBuilder;
    import com.santander.framework.springboot.core.context.Context;
    import com.santander.framework.springboot.core.context.ContextHolder;
    import com.santander.framework.springboot.core.context.Info;
    import com.santander.framework.springboot.events.CloudEvent;
    import java.net.URI;
    import java.time.OffsetDateTime;
...
The [ReactiveContextUtil](https://gluon.dev.corp/microservices/docs/santander-spring-boot/1.2.1/apidocs/com/santander/framework/springboot/events/reactive/ReactiveContextUtil.html)
...
will be available in the *downstream*. An example would be:
    ...
    import com.santander.framework.springboot.events.reactive.ReactiveContextUtil;
    import org.springframework.security.core.context.SecurityContext;
    import com.santander.framework.springboot.security.authentication.token.Token;
    import domain.com.santander.framework.springboot.security.authorization.AuthorizationToken;
    import reactor.core.publisher.Mono;
    ...
    public class ReactiveCloudEvent {
        ...
        public Mono<CloudEvent> getCloudEvent() {
                ...
                ReactiveContextUtil.getMonoContext()
                    .map(function(
                        (Context context, SecurityContext securityContext) -> {
                            ...
                            // Updating Santander Spring Boot Context
                            context.setInfo(info);
...
The second one allows to create the contexts with a default value. The context will be reported as an instance containing the desired values.
For the security context only an `Authentication` object can be provided and initialized:
    ...
    public class ReactiveCloudEvent {
        ...
        public Mono<CloudEvent> getCloudEvent() {
            ...
            Context dContext = Context.builder().build();
            Info info = Info.builder()
                                              .channel("channel")
                                              .entity("1234")
                                              .sessionId("abcd")
                ...
                ReactiveContextUtil.getMonoContext()
                    .map(function(
                        (Context context, SecurityContext securityContext) ->
                           ...
                           .withAuthentication(securityContext.getAuthentication())
                           .build()
                    )).contextWrite(contextModifier ->
                            ReactiveContextUtil.create(contextModifier, dContext, authentication)
                    );
        }
    }
...
!!! note
    ...
    To simplify the creation of the reactive contexts, both the security context and the Santander Spring Boot context, use is made of the
    [ReactiveContextUtil](https://gluon.dev.corp/microservices/docs/santander-spring-boot/1.2.1/apidocs/com/santander/framework/springboot/events/reactive/ReactiveContextUtil.html) utility class. This allows to subscribe the Reactor
...
[ReactiveContextUtil](https://gluon.dev.corp/microservices/docs/santander-spring-boot/1.2.1/apidocs/com/santander/framework/springboot/events/reactive/ReactiveContextUtil.html) class exposes a method that is responsible for
populating the Santander Spring Boot context and the security context from the headers reported in the events. The ***fillContext()*** method decomposes each of the messages and extracts the headers needed to populate each of the required fields.
In case the messages do not have the required information, the contexts will be initialized without any value (empty).
...
the event headers:
    ...
    import com.santander.framework.springboot.core.config.core.CoreProperties;
    import reactive.com.santander.framework.springboot.events.ReactiveContextUtil;
    import io.cloudevents.CloudEvent;
    import org.apache.kafka.common.header.Headers;
...
3. We inject the `Bean ReactiveContextUtil` that provides the Santander Spring Boot configuration. This allows easy creation and initialization of Santander Spring Boot and Security reactive contexts.

4. The HTTP client is configured to perform invocations to an external service.

5. The ***fillContext*** method of the [ReactiveContextUtil](https://gluon.dev.corp/microservices/docs/santander-spring-boot/1.2.1/apidocs/com/santander/framework/springboot/events/reactive/ReactiveContextUtil.html) class is used
...
8. We subscribe to the Reactor context for the generation of the Context and the SecurityContext. This creation should always be done manually to ensure that these will be available for initialization in the *upstream*.
...
[Kafka security guide](https://github.com/santander-group-shared-assets/gln-back-santander-java-samples/tree/develop/kafka/security)
