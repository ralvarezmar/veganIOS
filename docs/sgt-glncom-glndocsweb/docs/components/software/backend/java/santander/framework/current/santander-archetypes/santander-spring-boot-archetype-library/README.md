# Santander Spring Boot Library Archetype ![1.2.1](https://img.shields.io/badge/1.2.1-FF073D)

![Current](https://img.shields.io/badge/CURRENT-30FF24) ![GA](https://img.shields.io/badge/GA-C81D11)

## Description

The Maven archetype for Santander Spring Boot libraries allows the rapid creation of a 'skeleton' for Spring Boot libraries incorporating the necessary dependencies for their use.

## Dependency management

This project will serve as Parent POM for the Santander Spring Boot libraries based on Santander Spring Boot, providing dependencies and plugin management.
...
In the command line and from the directory in which you want to create the new project, you must execute a maven command like the following, substituting the value of the parameters for those of the project you want to create:
    ...
    mvn -B org.apache.maven.plugins:maven-archetype-plugin:3.3.1:generate -DarchetypeGroupId=com.santander.framework.springboot -DarchetypeArtifactId=santander-spring-boot-archetype-library -DarchetypeVersion={santander-version} -Dcomponent-name={component} -Dacronym-app={acronym} [-Dsystem={system} -Dsub-system-code={subsystem} -Dfunctional-application-code={application} -Dfunctional-sub-application-code={subapplication}]
...
- ***archetypeArtifactId***: Id of the archetype to be used, its value is `santander-spring-boot-archetype-library`.
...
!!! note
    ...
    This property corresponds to the AppKey of the Santander Spring Boot framework.
...
- ***description***: The default project description shall have the following text:
      ...
      ${acronym-app} santander spring boot library: ${component-name}
...
If valid values are used in the parameters, the expected project will be generated without any problem:
    ...
    mvn -B org.apache.maven.plugins:maven-archetype-plugin:3.3.1:generate -DarchetypeGroupId=com.santander.framework.springboot -DarchetypeArtifactId=santander-spring-boot-archetype-library -DarchetypeVersion=1.0.0-RELEASE -Dcomponent-name=greetings -Dacronym-app=santander
    ...
    [INFO] Archetype repository not defined. Using the one from [com.santander.framework.springboot:santander-spring-boot-archetype-library:1.0.0-RELEASE] found in catalog remote
    [INFO] ----------------------------------------------------------------------------
    [INFO] Using following parameters for creating project from Archetype: santander-spring-boot-archetype-library:1.0.0-RELEASE
    [INFO] ----------------------------------------------------------------------------
    [INFO] Parameter: groupId, Value: com.santander.framework.springboot
    [INFO] Parameter: artifactId, Value: greetings
    [INFO] Parameter: version, Value: 1.0.0-SNAPSHOT
    [INFO] Parameter: package, Value: com.santander.framework.springboot.greetings
    [INFO] Parameter: component-name, Value: greetings
    [INFO] Parameter: package, Value: com.santander.framework.springboot.greetings
    [INFO] Parameter: groupId, Value: com.santander.framework.springboot
    [INFO] Parameter: description, Value: Librería Spring Boot de santander: greetings
    [INFO] Parameter: artifactId, Value: greetings
    [INFO] Parameter: version, Value: 1.0.0-SNAPSHOT
    [INFO] Parameter: acronym-app, Value: santander
...
If we use the value "Greeting$" the following will occur:
    ...
    mvn -B org.apache.maven.plugins:maven-archetype-plugin:3.3.1:generate -DarchetypeGroupId=com.santander.framework.springboot -DarchetypeArtifactId=santander-spring-boot-archetype-library -DarchetypeVersion=1.0.0-RELEASE -Dcomponent-name=Greeting$ -Dacronym-app=santander
    [INFO] Scanning for projects...
    [INFO]
    [INFO] ------------------< org.apache.maven:standalone-pom >-------------------
    [INFO] Building Maven Stub Project (No POM) 1
    ...
    [INFO] Parameter: version, Value: 1.0.0-SNAPSHOT
    [INFO] Parameter: package, Value: com.santander.framework.springboot.Greeting$
    [INFO] Parameter: component-name, Value: Greeting$
    [INFO] Parameter: package, Value: com.santander.framework.springboot.Greeting$
    [INFO] Parameter: groupId, Value: com.santander.framework.springboot
    [INFO] Parameter: description, Value: Librería Spring Boot de santander: Greeting$
    [INFO] Parameter: artifactId, Value: Greeting$
    [INFO] Parameter: version, Value: 1.0.0-SNAPSHOT
    [INFO] Parameter: acronym-app, Value: santander
    [INFO] Executing META-INF/archetype-post-generate.groovy post-generation script
...
At the beginning it will always have the identification/metadata of the library we have created:
    ...
    <name>greetings</name>
    <description>myapp santander spring boot library: greetings</description>

The Santander Spring Boot framework defines the parent POM [mentioned previously](#dependency-management) to manage library versions so that there are no conflicts between them:

    <parent>
        <groupId>com.santander.framework.springboot</groupId>
        <artifactId>santander-spring-boot-dependencies</artifactId>
...
In the example above the content of this file would be as follows:
    ...
    com.santander.framework.springboot.greetings.config.HelloWorldAutoConfig

## Library use cases

Once we have created the Spring Boot project with the library, we are going to show a use case in an application.
For this example, we will rely on a Web application generated with the [Santander Spring Boot microservice archetype](../santander-spring-boot-archetype-microservice/README.md).
