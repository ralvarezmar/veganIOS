# Spring Santander Spring Boot Microservices Reference Guide ![1.2.1](https://img.shields.io/badge/1.2.1-FF073D)

![Current](https://img.shields.io/badge/CURRENT-30FF24) ![GA](https://img.shields.io/badge/GA-C81D11)

## Introduction

This document will give an overview of microservice development based on Santander Spring Boot Spring framework.
It is intended that by reading this document the developer knows what to do and what not.
This welcome pack will not go into detail since each framework library has its own documentation in which all the details are explained.
There are also guides with examples of different use cases.
...
The Santander Spring Boot microservice framework is based on the Spring ecosystem.
...
### Santander Spring Boot microservice framework components
...
- **Traces or 'Logging'**. Component that enriches the application traces to meet the format required by the Santander Spring Boot log platform.
  Automatically generates activity traces. Provides Helper classes so that microservices can write functional traces.
...
- **Super POM.** Santander Spring Boot microservice framework own dependency manager. Manages dependencies of the different libraries so that there are no incompatibilities between them.
...
The Santander Spring Boot framework is related to a series of external services that must be accessible to the microservice at runtime.
...
- **Santander Spring Boot Logging Platform.** The log traces generated by the microservices are directed to a centralized log aggregation system that allows the correct operation of the services in production.
  Integration with the logging platform is required for all microservices. Used by the module logging.

## First steps

### Prerequisites

The development environment to work with **Santander Spring Boot Spring Framework** is the usual one for any type of Java application.
...
The easiest way to start developing a Santander Spring Boot microservice is by using the microservice archetype.
...
- \* pom.xml: \* maven microservice descriptor. It already uses the parent of the Santander Spring Boot Spring framework and includes the dependencies required based on the selections made during the archetype execution.
...
See Santander Spring Boot archetypes documentation to know how to use it.
...
This section lists **mandatory** characteristics for java microservices in Santander Spring Boot architecture. Some of these features are described in greater detail in later sections.
...
Java microservices should be configured following the [Santander Spring Boot applications configuration guide](https://gluon.gs.corp/community/docs/latest/components/software/backend/microservice/standards/microservice-configuration/)

For this, the Santander Spring Boot microservices must include the dependency of the module **santander spring boot client.**

More information on the [Configuration section](#configuration) later in this document.

### Logging

Java microservices should write their log traces to the **Santander Spring Boot Log Platform.**

For this, the Santander Spring Boot microservices must include the dependency of the module **santander spring boot logging.**
...
For this, the microservices must include the dependency of the module **santander spring boot authentication.**

More information on the [Security section](#authentication) later in this document.

### Error management

Http error management should follow the instructions and have the format described in the [http services error handling guide](https://gluon.gs.corp/community/docs/latest/components/software/backend/microservice/standards/error-management/)

The library **santander spring boot core** offers utility classes to meet this requirement.
...
The architecture of a microservice is not different from that of a monolithic application. Its main difference is that it operates on a smaller domain and that its scope is in *backend*, which has no presentation layer.
It is therefore more focused on business logic and data persistence.

The microservice archetype also follows hexagonal architecture. This design pattern separates an application’s internal components into distinct layers with the goal of isolating business logic from external systems.
This adaptation to the hexagonal architecture for the microservice archetype was led by Mexico's Development Center of Excellence.

The main objective of hexagonal architecture is to allow the application to interact consistently with various clients—whether it’s a web application, a mobile app, or a REST API—while keeping the same business logic. Similarly, the application
can be connected to multiple databases or external services without modifying the core logic. This approach allows for independent development and testing of the application’s logic, free from concerns about external services or the user interface.

In hexagonal architecture, the application is divided into layers, each with a specific responsibility. This design is often visualized as a hexagon, with the business logic at the center and external services and interfaces on the sides.
The exact number of sides does not matter—what’s important is their role. Each side represents a “port,” facilitating input or output to and from the application. The business logic at the center remains independent of these ports.

The three main layers in hexagonal architecture are:

- ***Domain Layer***: This core layer contains the business logic, including entities, value objects, and services. It is independent of other layers.

- ***Application Layer***: This layer coordinates the domain layer with external services and interfaces. It contains the use cases that drive the application’s behavior.

- ***Infrastructure Layer***: Responsible for connecting the application to external services and interfaces, facilitating data input and output. It implements the ports of the application but does not contain business logic.

Each layer interacts through well-defined interfaces (ports) and implementations (adapters). This interaction must be carefully designed to ensure efficient and coherent communication between layers.

To build an application using hexagonal architecture, follow these best practices:

- **Separate concerns**: Each layer should have a specific responsibility, and the business logic should be isolated from external services.
- **Domain-driven design**: Focus on the core domain logic and model the application around it. The classes inside the domain layer should preferably be written in plain java.
- **Clear use cases**: Define the application’s behavior in the application layer, encapsulating the business logic in use cases.
- **Adapters for external services**: Implement adapters to connect the application to external services, such as databases, APIs, etc. in the infrastructure layer. These adapters should be interchangeable and independent of the business logic.

To get started with hexagonal architecture, first clearly define your domain. Identifying the entities and value objects that represent the core business concepts will help you build a solid foundation for your application.
Identify the core business logic and implement its required actions inside the domain service classes. Next, define the use cases that drive the application’s behavior in the application layer. Use cases are the goals the application must achieve.
Define the input and output ports, also at the application layer. Output ports define the interfaces that the application uses to communicate with external services.
Input ports implement the use case interfaces, and orchestrate the flow of information between the domain and infrastructure layers. They are used by input adapters to interact with external services.
Finally, implement the adapters in the infrastructure layer to connect the application to external services and interfaces.
Input adapters define the classes that allow users or systems to interact with the application and use input ports to execute use cases.
...
For example, a typical package structure for a servlet application with a database may be:
...
In other applications you may find more or less packages depending on the type and the modules chosen during the generation.
It is highly encouraged to review the Readme file documentation generated along with the application to take note of all the details of the microservice created.
...
The [recommendation](https://docs.spring.io/spring-framework/docs/6.2.11/reference/html/core.html#beans-factory-collaborators) by the developing team of Spring Framework:
...
In this way this POJO:
    ...
    package com.santander.framework.springboot.common.clientprofile;
...
documentation](https://docs.spring.io/spring-boot/docs/3.5.6/reference/html/features.html#features.external-config) defines the available mechanisms.

In Santander Spring Boot the following mechanisms can be used to configure java applications:
...
Detailed examples and guidelines for setting up java microservices are described within the following [document](https://gluon.gs.corp/community/docs/latest/components/software/backend/microservice/standards/microservice-configuration/).
It is necessary to adhere to what is described in this document to be able to deploy in successive environments.
...
**@ConfigurationProperties** indicates the prefix of the properties that Spring Boot will inject into the bean. For example in the previous case it will map the property *acme.health*
...
With some exceptions, microservices must check the user's credentials before executing a service. Santander Spring Boot Security is based on the JWT token.

This token is signed so the microservice must have the corresponding public key. To obtain this public key you must use a service called *Public Key Manager (PKM)*. There is a PKM deployed in the Santander Spring Boot Security Common Services application.
...
transformation. There is an STS deployed in the Santander Spring Boot Common Security Services application.

The Santander Spring Boot framework authentication library offers the following capabilities:
...
Details on the full functionality as well as its installation and configuration can be consulted in the module documentation [Santander Spring Boot Spring
Authentication](santander-project/santander-spring-boot-security-authentication/README.md).

## Logging and monitoring

### Logging

In order to be able to carry out an adequate *troubleshooting* of the Santander Spring Boot applications once they are
in production it is necessary that the components of said application generate their log information and dump it to the **Santander Spring Boot Logging platform.**
...
Consult [logging guide](https://santandernet.sharepoint.com/sites/SantanderPlatforms/SitePages/ObservabilityArch/OBS%20GLOBALOG%20previous%20versions/OBS-Fundamentals-GLOBALOG-v3_0_0-structure.aspx) for more details on the formats and requirements.

To facilitate its implementation, Santander Spring Boot Spring Framework includes a registry library which includes the following functionalities:
...
Details on the full functionality as well as its installation and configuration can be consulted in the module documentation [Santander Spring Boot Spring
Logging](santander-project/santander-spring-boot-logging/README.md).
...
More information about Acturator in the [Spring Boot documentation](https://docs.spring.io/spring-boot/docs/3.5.6/reference/html/actuator.html#actuator).
...
code and in the response body a standard format defined for Santander Spring Boot microservices.

In the article: [Http error treatment](https://gluon.gs.corp/community/docs/latest/components/software/backend/microservice/standards/error-management/)
The error codes to be entered and the format defined for the body of the response in case of error are explained.
...
The Santander Spring Boot Spring framework core library offers a series of utilities that help to implement this error handling. The library includes the following:
...
Consult the [framework documentation](santander-project/santander-spring-boot-core/README.md) to know the details of the functionalities and how to use
...
To know the complete functionality of Spring MVC consult the [documentation](https://docs.spring.io/spring-framework/docs/6.2.11/reference/html/web.html#spring-web) of the project.

Santander Spring Boot Spring Framework has integrations with Spring MVC in several of its modules: authentication, exception handling, authorization.

For the creation of a Web Controller please consult the [guide to building a web controller with Spring MVC](https://github.com/santander-group-shared-assets/gln-back-santander-java-samples/tree/develop/webcontroller).
...
[documentation](https://docs.spring.io/spring-framework/docs/6.2.11/reference/html/web-reactive.html#spring-webflux) of the project.
Santander Spring Boot Spring Framework has integrations with Spring WebFlux in several of its modules: authentication, exception handling, authorization.

For creating a reactive web controller, it is recommended to refer to the [guide to building a reactive web controller](https://github.com/santander-group-shared-assets/gln-back-santander-java-samples/tree/develop/reactivecontroller).
            ...
            */
            ...
            public GroupedOpenApi api() {
                return GroupedOpenApi.builder()
                    .setGroup("api-web")
                    .packagesToScan("com.santander.dwback.openapisample.infrastructure.adapters.input.rest")
                    .build();
            }
...
For more information on API documentation please refer to: [API Documentation with springDoc](https://github.com/santander-group-shared-assets/gln-back-santander-java-samples/tree/develop/openapi-springdoc)

## Synchronous communications

### Communication with other microservices via REST

Synchronous communication with other microservices is done using the http REST protocol.
The Santander Spring Boot microservices framework provides interceptors that are in charge of automatically propagating the headers related to *logging*, authentication and authorization.

To make a REST request to another microservice Spring Boot provides the *WebClient* class. The use of *WebClient* is recommended over *RestTemplate* because it performs better resource management. The Santander Spring Boot framework provides a *WebClient.Builder*
object that includes the interceptors for the architecture.

The following [guide](https://github.com/santander-group-shared-assets/gln-back-santander-java-samples/tree/develop/webclient) illustrates the use of *WebClient*.
...
To know the headers that Gluon architecture prescribes as necessary to enrich communication, it is recommended to read the [regulations on enriched
communication](https://gluon.gs.corp/community/docs/latest/components/software/api/framework/standards/design/headers/).
...
The *Santander Spring Boot Spring WebService* library is used to group several interceptors for authentication when communicating with SOAP services.
It currently includes an interceptor for authentication using a BKS token and an interceptor for user and password authentication.

To know the details about the functionalities and configuration of the library, consult its
[documentation](santander-project/santander-spring-boot-webservice/README.md).
...
**The use of the Circuit Breaker pattern is mandatory** in the Santander Spring Boot architecture whenever an external service is invoked synchronously.

The use of the *Resilience4J* library is recommended for the implementation of this pattern.

The guide [Circuit Breaker in Santander Spring Boot Spring](https://github.com/santander-group-shared-assets/gln-back-santander-java-samples/tree/develop/resilience) shows an example of how to implement *Circuit Breaker* in Santander Spring Boot microservices.
...
- **Spring for Apache Kafka:** For the integration with *Kafka.* Consult the [project documentation](https://docs.spring.io/spring-kafka/docs/3.3.10/reference/html).
...
Santander Spring Boot Spring Framework includes a cache module that incorporates connection dependencies to Datagrid and creates error handlers when accessing the cache.
...
documentation](santander-project/santander-spring-boot-cache/README.md)
...
![acceso\_bbd](images/acceso-bbdd.png)
...
The [BBDD SQL access guide](https://github.com/santander-group-shared-assets/gln-back-santander-java-samples/tree/develop/databasesql) illustrates the case of a database access using Spring Data JDBC.
...
!!! Note
    ...
    This apply only to Santander Spain
...
To access data with MongoDB, consult the [MongoDB access guide using SpringData Mongo](https://github.com/santander-group-shared-assets/gln-back-santander-java-samples/tree/develop/mongodb).

## Host access

Access to Banco Santander Host is done through the connectors for Partenon. There are two types of connectors: TrxOp and SAT.

In the following link you can consult the [Partenon connection library
documentation](https://santandernet.sharepoint.com/sites/SEPBKSCCA/SitePages/Annexes/Documentation/Spring%20Libraries%20Documentation/Partenon%20TrxOp%20Spring%20Connector%20-%20User%20Manual.aspx).
...
It is also recommended consulting the [access guide to Partenon in Santander Spring Boot microservices](https://github.com/santander-group-shared-assets/gln-back-santander-java-samples/tree/develop/partenon)
...
To learn more, please consult the guide to unit tests and integration in Santander Spring Boot:
[Testing for MVC applications](https://github.com/santander-group-shared-assets/gln-back-santander-java-samples/tree/develop/testing-mvc) and [Testing for Reactive applications](https://github.com/santander-group-shared-assets/gln-back-santander-java-samples/tree/develop/testing-webflux).
