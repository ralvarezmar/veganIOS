# Santander Spring Boot Omnichannel ![1.2.1](https://img.shields.io/badge/1.2.1-FF073D)

![Current](https://img.shields.io/badge/CURRENT-30FF24) ![GA](https://img.shields.io/badge/GA-C81D11)

## Description

The `Santander Spring Boot Omnichannel` library allows storing and manage all the configurable properties associated to each supported channel within the Banco Santander architecture.
...
- A `SantanderClientHttpRequestInterceptor` named `OmniChannelInterceptor` is injected in charge of adding/propagating the HTTP header ***Contact-Point*** with the corresponding information:
...
!!! tip "Caution"
    ...
    It is necessary for the project to register a `RestTemplate` type bean and stereotype it with the `@SantanderQualifier` annotation for the instrumentation to be applied.

This functionality is completely transparent to the user, if the project registers a bean of type `RestTemplate` without the `@SantanderQualifier` annotation,
it is understood that you want to use your own `RestTemplate` instead of the one configured
by `Santander Spring Boot` so the instrumentation will **NOT** be applied.
...
1. We inject the `RestTemplate` bean annotated with **@SantanderQualifier**.
...
object, which is located in `Context`.

#### Omnichannel Web Filter

A `GenericFilterBean` bean is exposed to fill the `ContactPoint` of the **Context** with the information associated to the request's channel and *Contact-Point*.
...
A `OncePerRequestFilter` bean is exposed to fill the `ContactPoint` of the **Context** with the information associated to the request's *User-Agent*.
...
from the `ContactPoint` object, which is located in `Context`.

#### Omnichannel Web Filter

A `OrderedWebFilter` Bean is exposed to populate the `ContactPoint` of the **Reactive Context** with the information associated to the request's channel and *Contact-Point*.
...
A `OrderedWebFilter` bean is exposed to fill the `ContactPoint` of the **Reactive Context** with the information associated to the request's *User-Agent*.
...
If we want to add the functionality to parse the User-Agent header and get the **device** and **operatingSystem** fields, we will have to add the following dependency:
    ...
    <dependency>
        <groupId>com.santander.framework.springboot</groupId>
        <artifactId>santander-spring-boot-starter-omnichannel-ua-parser</artifactId>
    </dependency>

!!! warning "Warning"

    The use of the **santander-spring-boot-starter-omnichannel-ua-parser** dependency leads to a memory consumption of 100Mb, so we only recommend its use when it is essential to obtain the **device** and **operatingSystem** fields.
...
|---------------------------------------|----------------------------------|----------|--------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------|----------------------------------------------------------|
| santander.omnichannel.header.entity      | <code>organization</code>        | No       | Name of the HTTP header for the entity                                                                                                                 | String                           | <ul><li><p>Servlet</p></li><li><p>Reactive</p></li></ul> |
| santander.omnichannel.header.channel     | <code>X-Santander-Channel</code> | No       | Name of the HTTP header for the channel                                                                                                                | String                           | <ul><li><p>Servlet</p></li><li><p>Reactive</p></li></ul> |
| santander.omnichannel.parameter.entity   | entity                           | No       | Parameter where the entity will be fetched if the <code>organization</code> header does not exist or is null and if it is not found in the URL         | String                           | <ul><li><p>Servlet</p></li><li><p>Reactive</p></li></ul> |
| santander.omnichannel.parameter.channel  | channel                          | No       | Parameter where the channel will be fetched if the <code>X-Santander-Channel</code> header does not exist or is null and if it is not found in the URL | String                           | <ul><li><p>Servlet</p></li><li><p>Reactive</p></li></ul> |
| santander.omnichannel.externalChannelMap | -                                | No       | Map to overwrite the original channel information or define new channels                                                                               | <String, <String, ContactPoint>> | <ul><li><p>All</p></li></ul>                             |
| santander.omnichannel.filter.order       | -15                              | No       | Order in which the omnichannel filter is placed in the chain                                                                                           | Number                           | <ul><li><p>All</p></li></ul>                             |
...
This initial configuration can be replaced by another one decided by the project, for that the only thing to do is to define a file named `santanderchannels.json` and place it in the `resources` section, that is to say, anywhere below
...
On the other hand, the `omnichannel` library will leave an INFO type trace in the application log with the initial configuration applied. The specific trace will be:
    ...
    Santander OmniChannel config = {ALL={OFI=ContactPoint(originChannel=OFI, marcoChannel=OFI, environment=INTRANET, logicalChannel=0, physicalChannel=0, commercialChannel=RED, operationalChannel=RED, application=null, device=null, operatingSystem=null, medium=null, userType=null), INT=ContactPoint(originChannel=INT,...  etc.

In addition, this startup information can be rewritten by the developer to add and/or modify the channels he/she deems appropriate through the `santander.omnichannel.externalChannelMap` property. For this refer to the following
...
For more information about the Propagation Headers visit [more information](../santander-spring-boot-logging/README.md#header-propagation)
...
This library includes tentative support for *yauaa* library.
...
| [HttpWebChannelReader](https://gluon.dev.corp/microservices/docs/santander-spring-boot/1.2.1/apidocs/com/santander/framework/springboot/omnichannel/readers/HttpWebChannelReader.html)           | [HttpReader](https://gluon.dev.corp/microservices/docs/santander-spring-boot/1.2.1/apidocs/com/santander/framework/springboot/omnichannel/readers/HttpReader.html)            | CAuxiliary class with which the entity and the channel can be extracted from the header or parameter of a request.                                                          | Servlet          |
| [HttpReactiveChannelReader](https://gluon.dev.corp/microservices/docs/santander-spring-boot/1.2.1/apidocs/com/santander/framework/springboot/omnichannel/readers/HttpReactiveChannelReader.html) | [HttpReader](https://gluon.dev.corp/microservices/docs/santander-spring-boot/1.2.1/apidocs/com/santander/framework/springboot/omnichannel/readers/HttpReader.html)            | Auxiliary class with which the entity or channel can be extracted from the header or parameter of a request.                                                                | Reactive         |
| [OmniChannelFilter](https://gluon.dev.corp/microservices/docs/santander-spring-boot/1.2.1/apidocs/com/santander/framework/springboot/omnichannel/filter/OmniChannelFilter.html)                  | [GenericFilterBean](https://docs.spring.io/spring-framework/docs/6.2.11/javadoc-api/org/springframework/web/filter/GenericFilterBean.html)    | OmniChannel Filter for Servlet applications.                                                                                                                                | Servlet          |
| [OmniChannelWebFilter](https://gluon.dev.corp/microservices/docs/santander-spring-boot/1.2.1/apidocs/com/santander/framework/springboot/omnichannel/filter/OmniChannelWebFilter.html)            | [WebFilter](https://docs.spring.io/spring-framework/docs/6.2.11/javadoc-api/org/springframework/web/server/WebFilter.html)                    | OmniChannel Filter for Reactive applications.                                                                                                                               | Reactive         |
| [UserAgentFilter](https://gluon.dev.corp/microservices/docs/santander-spring-boot/1.2.1/apidocs/com/santander/framework/springboot/omnichannel/filter/UserAgentFilter.html)                      | [GenericFilterBean](https://docs.spring.io/spring-framework/docs/6.2.11/javadoc-api/org/springframework/web/filter/OncePerRequestFilter.html) | UserAgent Filter for Servlet applications.                                                                                                                                  | Servlet          |
| [UserAgentWebFilter](https://gluon.dev.corp/microservices/docs/santander-spring-boot/1.2.1/apidocs/com/santander/framework/springboot/omnichannel/filter/UserAgentWebFilter.html)                | [WebFilter](https://docs.spring.io/spring-framework/docs/6.2.11/javadoc-api/org/springframework/web/server/WebFilter.html)                    | UserAgent Filter for Reactive applications.                                                                                                                                 | Reactive         |
...
!!! info "Important"
    ...
    This functionality is exclusive for `Servlet` type applications. If the `Santander Spring Boot` authentication library is used, it is also possible to retrieve the `ContactPoint` object from the security context. This example details how to
    do it:
...
!!! info "Important"
    ...
    In future versions this functionality will be removed but for the moment it is kept for backward compatibility. &lt;&lt;santander-context, The recommendation is to retrieve the `ContactPoint` from the `Context`.

### How to retrieve the channel from the Context

#### Non-reactive environments

The `ContextHolder` object is initialized by the OmniChannel filter (`OmniChannelFilter`) so to retrieve the channel information you have to use code like this:

    ...
    ...
    ContactPoint contactPoint = ContextHolder.getCurrentContext().getContactPoint();
    if (contactPoint != null ) {
            ...
        }
    ...

This code will obtain the `ContactPoint` object from the `Context`.

#### Reactive environments

In the case of reactive environments, the `Context` object is initialized by the `ReactiveWebFilter`, so to retrieve the channel information you have to use a code like this:

    Mono<ContactPoint> contactPoint = ReactiveContextHolder.getContext() (1)
...
This functionality is no longer part of this module, the equivalent example is in the [Core: Propagate headers in NotWeb applications](../santander-spring-boot-core/README.md#header-propagation-in-notweb-applications) module.
...
!!! info "Important"
    ...
    A Santander Spring Boot microservice always propagate the `Contact-Point` header with this format.
...
3. **No header received**
    ...
    !!! note
        ...
        **device** and **operatingSystem** fields are filled only if santander-spring-boot-starter-omnichannel-ua-parser is used 
...
In servlet applications:
    ...
    ContactPoint contactPoint = ContextHolder.getCurrentContext().getContactPoint();
    ...
    context.setContactPoint(contactPoint);
    }

In reactive applications:

    Mono<ContactPoint> contactPoint = ReactiveContextHolder.getContext()
...
!!! note
    ...
    The interceptors we inject into `RestTemplate` and/or `WebClient` make use of `Context` to retrieve information from the `ContactPoint` or the header of the same name. In `Web` applications, `Context` is
    initialized thanks to `Web` filters that `Santander Spring Boot` creates and initializes; however, in `NotWeb` applications we do not have such filters, so the `Context` object is not automatically initialized.
