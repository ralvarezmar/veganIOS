# Santander Spring Boot WebService ![1.2.1](https://img.shields.io/badge/1.2.1-FF073D)

![Current](https://img.shields.io/badge/CURRENT-30FF24) ![GA](https://img.shields.io/badge/GA-C81D11)

## Description

The `Santander Spring Boot WebService` library was created to facilitate the sending of SOAP requests to Web Services, providing mechanisms to:
...
The `Santander Spring Boot WebService` library provides the following functionalities depending on the application type:
...
Exposes a bean with name `santanderWSRequestHelper` and type `WSRequestHelper`, the functions of which are:
...
!!! info "Important"
    ...
    Note that in order to use the `BinarySecurityToken` tokens, the application must have the authentication library as a dependency, therefore `Santander Spring Boot Starter Webservice` automatically imports the
    `Santander Spring Boot Starter Authentication`.

| Application Type            | Environment | Dependencies                                                                                                                                                                                                                                                 |
|-----------------------------|-------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| WebApplicationType.NONE     | NotWeb      | <ul><li>com.santander.framework.springboot:santander-spring-boot-starter-webservice</li></ul>                                                                                                                                                                                 |
| WebApplicationType.SERVLET  | Servlet     | <ul><li>com.santander.framework.springboot:<strong>santander-spring-boot-starter-webservice</strong></li><li>org.springframework.boot:<strong>spring-boot-starter-web</strong></li><li>org.springframework.boot:<strong>spring-boot-actuator-autoconfigure</strong></li></ul> |
| WebApplicationType.REACTIVE | Reactive    | <ul><li>com.santander.framework.springboot:<strong>santander-spring-boot-starter-webservice</strong></li><li>org.springframework.boot:<strong>spring-boot-actuator-autoconfigure</strong></li></ul>                                                                           |
...
|---------------------------------------------------------------------------|---------------|------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------|
| santander.webservices.security-types                                         | N/A           | Yes                          | Types of security to be applied throughout SOAP clients                                                                                                                                            | List                                                          |
| santander.webservices.security-types(i).name                                 | N/A           | Yes                          | Value that is mapped against the attribute "santander.webservices.security-types.mode" attribute so that each client knows which one to use.                                                       | String                                                        |
| santander.webservices.security-types(i).mode                                 | N/A           | Yes                          | BinarySecurityToken to authenticate using corporate token and UsernameToken to authenticate by username/password. Any other value will be interpreted as not putting security headers in the call. | <ul><li>Binary SecurityToken</li><li>Username Token</li></ul> |
| santander.webservices.security-types(i).user                                 | N/A           | Only if mode = UsernameToken | UserId if mode is UsernameToken.                                                                                                                                                                   | String                                                        |
| santander.webservices.security-types(i).password                             | N/A           | Only if mode = UsernameToken | Password if mode is UsernameToken.                                                                                                                                                                 | String                                                        |
...
!!! note
    ...
    In this example the CLIENT\_PROFILES\_PATH has been defined as `santander.webservices`.
...
| Name                                                          | Default value | Required                           | Description                                                                                                                                                                                    | Type                                                          |
|---------------------------------------------------------------|---------------|------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------|
| santander.webservices.security-types                             | N/A           | No                                 | Types of security to be applied throughout SOAP clients.                                                                                                                                       | List                                                          |
| santander.webservices.security-types(i).name                     | N/A           | Only if security-types is defined  | Identifier to be used when this type of security is to be used.                                                                                                                                | String                                                        |
| santander.webservices.security-types(i).mode                     | N/A           | Only if security-types are defined | BinarySecurityToken to authenticate using corporate token and UsernameToken to authenticate by user/password. Any other value will be interpreted as not putting security headers in the call. | <ul><li>Binary SecurityToken</li><li>Username Token</li></ul> |
| santander.webservices.security-types(i).user                     | N/A           | Only if mode = UsernameToken       | UserId if mode is UsernameToken.                                                                                                                                                               | String                                                        |
| santander.webservices.security-types(i).password                 | N/A           | Only if mode = UsernameToken       | Password if mode is UsernameToken.                                                                                                                                                             | String                                                        |
| santander.webservices.client-profiles                            | N/A           | No                                 | Set of possible client profiles to be used to choose the URL of the WebService according to the channel.                                                                                       | Set                                                           |
| santander.webservices.client-profiles(i).name                    | N/A           | Only if client-profiles            | Client profile identifier is defined.                                                                                                                                                          | String                                                        |
| santander.webservices.client-profiles(i).defaultEndpoint         | N/A           | Only if client-profiles is defined | The default endpoint to be requested from if no other endpoint is fetched via the channel.                                                                                                     | String                                                        |
| santander.webservices.client-profiles(i).marcoChannels           | N/A           | No                                 | Map with the channel (key) and endpoint (value) pairs that are defined in this profile.                                                                                                        | Map                                                           |
| santander.webservices.client-profiles(i).marcoChannels.{channel} | N/A           | No                                 | Endpoint to be returned if the channel retrieved from the context matches {channel}.                                                                                                           | String                                                        |
...
This module is partially supported in native compilation. This module works only in reactive microservices.

!!! note

    This module does not work in native mode for servlet type applications.

## Exposed API

| Name                                                                                                                                                                                                         | Type                                                                                                                                                               | Description                                                                                                                                                                                                     | Application Type                          |
|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------|
| interceptorsBean                                                                                                                                                                                             | @Bean Map<String, [ClientInterceptor](https://docs.spring.io/spring-ws/docs/current/api/org/springframework/ws/client/support/interceptor/ClientInterceptor.html)> | Dictionary with the appropriate interceptor for the different security types defined in "santander.webservices.security-types"                                                                                  | <ul><li>NotWeb</li><li>Servlet</li></ul>  |
| [WebEndPointSelectorService](https://gluon.dev.corp/microservices/docs/santander-spring-boot/1.2.1/apidocs/com/santander/framework/springboot/ws/service/WebEndPointSelectorService.html) | @Bean                                                                                                                                                              | Service to get the endpoint of the Webservice according to the channel                                                                                                                                          | <ul><li>NotWeb</li><li>Servlet</li></ul>  |
| [WSRequestHelper](https://gluon.dev.corp/microservices/docs/santander-spring-boot/1.2.1/apidocs/com/santander/framework/springboot/ws/helper/WSRequestHelper.html)            | @Bean                                                                                                                                                              | Helper class for creating SOAP requests in WebClient with encapsulated objects and being able to select the encapsulated objects and to be able to select the target endpoint according to the current channel. | <ul><li>NotWeb</li><li>Reactive</li></ul> |
...
Below is an example of a Java class, `SoapObject`, whose instances will serve as payload (WebService input parameter) in the request of the examples:
    ...
    @XmlRootElement(namespace = com.santander.framework.springboot.test.micro.service.ws.SoapObject.NAMESPACE_URI, name = "soapObject")
    public class SoapObject {

       public static final String NAMESPACE_URI = "http://santander/security-web-service";
...
The following Java class shows an example of use in a controller.
    ...
    public class WebserviceController {
       ...
       @Value("${santander.security.test.webservice-request-url}")
...
that collects the information from the `ResourcesBean` class:
    ...
    @ConfigurationProperties(prefix = "santander.webservices.client1")
    @Data
...
For this, it will be necessary to define other configuration classes different from the previous case: a POJO class with all the client information and another one that collects the client map of the properties:
    ...
    @Data
    @ConfigurationProperties(prefix = "santander.webservices")
...
To use the `WebService` library via `WebClient` (instead of `WebServiceTemplate`), a self-configured `WSRequestHelper` class bean is available.
...
calling a `WebService`:
    ...
    public class WebserviceReactiveController {
        ...
        private WSRequestHelper santanderWSRequestHelper;

        private WebClient.Builder webClientBuilder;

        public void WebserviceReactiveController(WSRequestHelper santanderWSRequestHelper,
                                                    WebClient.Builder webClientBuilder){
            this.santanderWSRequestHelper = santanderWSRequestHelper;
        ...
        public Mono<WsResponse> getResponse() {
            ...
            return santanderWSRequestHelper.encodeInput(soapObject, securityType)
                    .flatMap(data ->
...
microservice that invokes a WebService:
    ...
    public class WebserviceReactiveController {
        ...
        public Mono<WsResponse> getResponse() {
            ...
            return santanderWSRequestHelper
...
[Authentication library](../santander-spring-boot-security-authentication/README.md#tokenservice).
...
The following rewrites the example [Use of the library interceptors](#use-of-the-library-interceptors) for a `NotWeb` application:
    ...
    import com.santander.framework.springboot.security.authentication.service.SecurityManagerService;
...
When dealing with a `NotWeb` application we have the same functionality as in [Reactive](#web-service-invocations-in-reactive-applications) (WSRequestHelper); however, in case we want to use a tokenised security header,
before invoking the Web Service we will have to use the **reactive implementation** of the `SecurityManagerService` defined in the [Authentication library](../santander-spring-boot-security-authentication/README.md#tokenservice).
...
The following rewrites the example [Defining the request body and URL in a WebClient](#defining-the-request-body-and-url-in-a-webclient) for a `NotWeb` application:
    ...
    public class WebserviceReactiveCaller {
        ...
        public void WebserviceReactiveCaller(WSRequestHelper santanderWSRequestHelper,
                                                    WebClient.Builder webClientBuilder,
        ...
        public Mono<WsResponse> getResponse(String stringToken) {
            ...
            return authResult.flatMap(
                        authToken ->
                            santanderWSRequestHelper.encodeInput(soapObject, securityType)     // (4)
                                        .flatMap(data ->
