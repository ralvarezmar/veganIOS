# Santander Spring Boot Authentication ![1.2.1](https://img.shields.io/badge/1.2.1-FF073D)

![Current](https://img.shields.io/badge/CURRENT-30FF24) ![GA](https://img.shields.io/badge/GA-C81D11)

## Description

The `Santander Spring Boot Boot Security Authentication` library serves as a wrapper for the Spring Framework with the objective of providing a seamless integration between it and the applications developed in the Banco Santander. This integration
...
It is based on **Spring Security** and automatically implements the following interfaces either with Santander Spring Boot's own implementations extending those provided by Spring Security or using Spring Security implementations directly:
...
- `Converter<Jwt, AbstractAuthenticationToken>`, `Converter<Jwt, Mono<AbstractAuthenticationToken>>`: Converts the JWT token into the Spring Authentication object, in our case the Santander Spring Boot implementation `AuthenticationBearerToken`
...
In the case of **token translation service (STS) invocations**, Santander Spring Boot also caches the requests, thus reducing the number of accesses to the service. To do this, it uses a **caffeine/DataGrid in-memory cache named "credentials "**,
where a maximum dwell time of 10 minutes is set.
...
- **Authorization of a request for a user** through the use of the Channel/Business Operational Control. This functionality prior token authentication is also included in this functionality, for more information on authorization: [Authorization
    library](../santander-spring-boot-security-authorization/README.md).
...
#### Extract JWT Token claims to Context

If the microservice **is called with a JWT token** it will look for some claims to update the Context:

- `channel_tp` claim -> Info.`channel`
- `cid_tp` claim -> Info.`appInit`
- `client_id` claim -> Context.`JWTClientId`
- `coreUserId` claim -> Context.`coreUserId`
...
|-----------------------------|-----------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| WebApplicationType.NONE     | NotWeb                | <ul><li>com.santander.framework.springboot:<strong>santander-spring-boot-starter-authentication</strong></li></ul>                                                                                                                                                                |
| WebApplicationType.SERVLET  | Servlet               | <ul><li>com.santander.framework.springboot:<strong>santander-spring-boot-starter-authentication</strong></li><li>org.springframework.boot:<strong>spring-boot-starter-web</strong></li><li>org.springframework.boot:<strong>spring-boot-actuator-autoconfigure</strong></li></ul> |
| WebApplicationType.REACTIVE | Reactive              | <ul><li>com.santander.framework.springboot:<strong>santander-spring-boot-starter-authentication</strong></li><li>org.springframework.boot:<strong>spring-boot-actuator-autoconfigure</strong></li></ul>                                                                           |

!!! info "Important"

    Although you can add the dependencies and application type manually, the Santander Spring Boot Archetypes has been redesigned to include all possible
...
|-----------------------------------------------------------|--------------------|----------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------------|--------------------------------------------|
| santander.security.enabled                                   | true               | No       | Indicates whether to initialise the authentication library or not.                                                                                                         | Boolean          | <ul><li>All</li></ul>                      |
| santander.security.white-list                                | N/A                | No       | List of routes, in AntPath format, that do not require authentication.                                                                                                     | List             | <ul><li>Servlet</li><li>Reactive</li></ul> |
| santander.security.auth-query-parameter                      | token              | No       | Sets the name of the parameter in which the authentication token can arrive.                                                                                               | String           | <ul><li>Servlet</li><li>Reactive</li></ul> |
| santander.security.audience                                  | N/A                | No       | If this parameter is informed, at least one recipients of the list must be contained in the JwtClaimNames.AUD field of the token for this token to be correctly validated. | String           | <ul><li>All</li></ul>                      |
| santander.security.issuer                                    | N/A                | No       | Issuer of the token. Corresponds to the claim "iss" of the JWT token. If this value is not null, only tokens from a given issuer will be accepted.                         | String           | <ul><li>All</li></ul>                      |
| santander.security.bks.enabled                               | true               | No       | Enables BKS token support. BKS Token is only supported if it is using <strong>PKM server</strong> to validate tokens.                                                      | Boolean          | <ul><li>Servlet</li><li>Reactive</li></ul> |
| santander.security.content-security-policy.enabled           | false              | No       | Enables the Content Security Policy (CSP) in the application.                                                                                                              | Boolean          | <ul><li>Servlet</li><li>Reactive</li></ul> |
| santander.security.content-security-policy.policy-directives | default-src 'self' | No       | The Content Security Policy (CSP) policy directives.                                                                                                                       | String           | <ul><li>Servlet</li><li>Reactive</li></ul> |

#### STS Connector

| Name                                                             | Default value | Required | Description                                                                                                                                                                                                                                                    | Supported values | Environment           |
|------------------------------------------------------------------|---------------|----------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------------|-----------------------|
| santander.security.connectors.sts-connector.sts-endpoint            | N/A           | No       | Path to the token conversion service (STS). It is only necessary if the microservice needs to convert BKS tokens to JWT or vice versa. Up to two routes can be defined in case of failure of the first one.                                                    | List             | <ul><li>All</li></ul> |
| santander.security.connectors.sts-connector.connect-timeout         | 1000          | No       | Sets the time in milliseconds to wait before dropping the connection. A value of 0 indicates an infinite timeout.                                                                                                                                              | Number           | <ul><li>All</li></ul> |
| santander.security.connectors.sts-connector.pending-acquire-timeout | 1000          | No       | Sets the timeout time in milliseconds used when requesting a connection from the connection manager using the underlying HttpClient object. A value of 0 indicates an infinite timeout.                                                                        | Number           | <ul><li>All</li></ul> |
| santander.security.connectors.sts-connector.read-timeout            | 1000          | No       | Sets the socket read timeout for the underlying HttpClient object. A value of 0 indicates an infinite timeout.                                                                                                                                                 | Number           | <ul><li>All</li></ul> |
| santander.security.connectors.sts-connector.max-life-time           | 60000         | No       | Sets the maximum persistence lifetime of connections. No value keeps the connection alive.                                                                                                                                                                     | Number           | <ul><li>All</li></ul> |
| santander.security.connectors.sts-connector.wiretap                 | false         | No       | Enable the wiretap to each request and response will be logged in full detail. To logging with Netty HttpClient also we have to set the log level of Netty's client package reactor.netty.http.client to DEBUG:1 logging.level.reactor.netty.http.client=DEBUG | Boolean          | <ul><li>All</li></ul> |

#### PKM Connector

| Name                                                             | Default value | Required | Description                                                                                                                                                                                                                                                                              | Supported values | Environment           |
|------------------------------------------------------------------|---------------|----------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------------|-----------------------|
| santander.security.connectors.pkm-connector.pkm-endpoint            | N/A           | No       | Path to the public key service. Up to two routes can be defined in case of failure of the first one.                                                                                                                                                                                     | List             | <ul><li>All</li></ul> |
| santander.security.connectors.pkm-connector.connect-timeout         | 1000          | No       | Sets the time in milliseconds to wait before dropping the connection. A value of 0 indicates an infinite timeout.                                                                                                                                                                        | Number           | <ul><li>All</li></ul> |
| santander.security.connectors.pkm-connector.pending-acquire-timeout | 1000          | No       | Sets the timeout in milliseconds used when requesting a connection from the connection manager using the underlying HttpClient object. used when requesting a connection from the connection manager using the underlying HttpClient object. A value of 0 indicates an infinite timeout. | Number           | <ul><li>All</li></ul> |
| santander.security.connectors.pkm-connector.read-timeout            | 1000          | No       | Sets the socket read timeout for the underlying HttpClient object. A value of 0 indicates an infinite timeout.                                                                                                                                                                           | Number           | <ul><li>All</li></ul> |
| santander.security.connectors.pkm-connector.max-life-time           | 60000         | No       | Sets the maximum persistence lifetime of connections. No value keeps the connection alive.                                                                                                                                                                                               | Number           | <ul><li>All</li></ul> |
| santander.security.connectors.pkm-connector.wiretap                 | false         | No       | Enable the wiretap to each request and response will be logged in full detail. To logging with Netty HttpClient also we have to set the log level of Netty's client package reactor.netty.http.client to DEBUG: <code>logging.level.reactor.netty.http.client=DEBUG</code>               | Boolean          | <ul><li>All</li></ul> |
...
| spring.security.oauth2.resourceserver.jwt.issuer-uri                              | N/A           | No       | Issuer of the token. Corresponds to the claim "iss" of the JWT token. If this value is not null, only tokens from a given issuer will be accepted. If the parameter santander.security.issuer is defined then it has precedence                                                          | String           | <ul><li>All</li></ul> |
| santander.security.connectors.authorization-server-connector.connect-timeout         | 1000          | No       | Sets the time in milliseconds to wait before dropping the connection. A value of 0 indicates an infinite timeout.                                                                                                                                                                        | Number           | <ul><li>All</li></ul> |
| santander.security.connectors.authorization-server-connector.pending-acquire-timeout | 1000          | No       | Sets the timeout in milliseconds used when requesting a connection from the connection manager using the underlying HttpClient object. used when requesting a connection from the connection manager using the underlying HttpClient object. A value of 0 indicates an infinite timeout. | Number           | <ul><li>All</li></ul> |
| santander.security.connectors.authorization-server-connector.read-timeout            | 1000          | No       | Sets the socket read timeout for the underlying HttpClient object. A value of 0 indicates an infinite timeout.                                                                                                                                                                           | Number           | <ul><li>All</li></ul> |
| santander.security.connectors.authorization-server-connector.max-life-time           | 60000         | No       | Sets the maximum persistence lifetime of connections. No value keeps the connection alive.                                                                                                                                                                                               | Number           | <ul><li>All</li></ul> |
        ...
        oauth2:
          resourceserver:
            jwt:
              ...
              issuer-uri: issuer     # Issuer accepted by application. This parameter is also accepted but santander.security.issuer has precedence
...
|---------------------------------------------------------------------------------------------------------------------------------------------------|------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------|
| [SecurityManagerService&lt;Authentication,Token>](https://gluon.dev.corp/microservices/docs/santander-spring-boot/1.2.1/apidocs/com/santander/framework/springboot/security/authentication/service/SecurityManagerService.html)                    | SecurityManagerService | Service exposing the mandatory implementation of the main functions of the security libraries.                                                                                                                                                                                                                                                                                                                                                         | <ul><li>NotWeb</li></ul>                  |
| [SecurityManagerService&lt;Mono&lt;Authentication>, Mono&lt;Token>>](https://gluon.dev.corp/microservices/docs/santander-spring-boot/1.2.1/apidocs/com/santander/framework/springboot/security/authentication/service/SecurityManagerService.html) | SecurityManagerService | Service exposing the reactive implementation of the main functions of the libraries security libraries.                                                                                                                                                                                                                                                                                                                                                | <ul><li>NotWeb</li></ul>                  |
| [TokenService&lt;String, Token>](https://gluon.dev.corp/microservices/docs/santander-spring-boot/1.2.1/apidocs/com/santander/framework/springboot/security/authentication/service/TokenService.html)                                               | TokenService           | Service that allows to obtain tokens of type BKS or JWT at any point of the non-reactive application and verify a given Token. To obtain tokens it may use STS Service to convert a token found of different type.                                                                                                                                                                                                                                     | <ul><li>NotWeb</li><li>Servlet</li></ul>  |
...
By default, Santander Spring Boot validates the timestamps of JWT tokens and optionally the issuers and audiences (configurable by parameters). If we want to customize we can define a bean of type `OAuth2TokenValidator<Jwt>` in our application.
...
Santander Spring Boot adds a `ReactiveJwtDecoder` or `JwtDecoder` bean depending on the type of application (reactive or servlet). This bean validates tokens against an **authorization server** or a **PKM service**, depending on how it is configured.
...
**Example of ReactiveJwtDecoder customization. Reactive application.**
    ...
    ReactiveJwtDecoder decoder(OAuth2TokenValidator<Jwt> validator) throws NoSuchAlgorithmException, InvalidKeySpecException {
        ...
        // Inject Santander Spring Boot validator (or application customize validator)
...
Santander Spring Boot adds the `Converter<Jwt, Collection<GrantedAuthority>>` santanderJwtToGrandAuthoritiesConverter or `Converter<Jwt, Flux<GrantedAuthority>>` santanderReactiveJwtToGrandAuthoritiesConverter object.
These objects are responsible for create the user's GrantedAuthorities in the `Authenticated` object. By default it creates them from the **scp** or **scope** claims of the JWT token.

This object can be customized by the application.

!!! warning

    It is important to use exactly the name santanderJwtToGrandAuthoritiesConverter or santanderReactiveJwtToGrandAuthoritiesConverter, because these beans are injected by name.
...
For **Reactive application**:
    ...
    @Bean
    Converter<Jwt, Flux<GrantedAuthority>> santanderReactiveJwtToGrandAuthoritiesConverter() {
...
For **Servlet application**:
    ...
    @Bean
    Converter<Jwt, Collection<GrantedAuthority>> santanderJwtToGrandAuthoritiesConverter() {
...
For **Reactive application**:
    ...
    @Bean
    ServerAuthenticationConverter santanderBearerConverter(AuthenticationProperties authenticationProperties) {
...
For **Servlet application**:
    ...
    BearerTokenResolver santanderBearerTokenResolver() {
...
!!! warning
    ...
    It is important to use exactly the name santanderJwtToAuthenticationConverter or santanderReactiveJwtToAuthenticationConverter, because these beans are injected by name.
...
For **Reactive application**:
    ...
    @Bean
    Converter<Jwt, Mono<AbstractAuthenticationToken>> santanderReactiveJwtToAuthenticationConverter() {
...
For **Servlet application**:
    ...
    @Bean
    Converter<Jwt, AbstractAuthenticationToken> santanderJwtToAuthenticationConverter() {
...
With the name santanderAuthorizationConfiguration a bean of type HttpSecurityCustomizer or ReactiveHttpSecurityCustomizer
is injected that allows changing the authorization configuration to perform a customized authorization.

!!! warning

    It is important to use exactly the name santanderAuthorizationConfiguration, because these beans are injected by name.
...
- **Validate/Authenticate a Token**
    ...
    2. If it is not null and it is authenticated, we convert it to the type defined by Santander Spring Boot: `AuthenticationBearerToken`.
...
- [Authorising access to a service with a token](../santander-spring-boot-security-authorization/README.md#exclusive-notweb-authorization-services)
...
!!! tip "Caution"
    ...
    To use the token conversion service (STS) it is necessary to define at least one path to an STS in the property **santander.security.connectors.sts-connector.sts-endpoint**.
...
However, if when invoking either of them they are not available or we haven't defined the path to any STS (**santander.security.connectors.sts-connector.sts-endpoint**) and you need to convert the one you have, a null or MonoEmpty will be returned. An
...
If you need to disable the library, preventing the installation of Santander Spring Boot's own configuration, you need to set the following property to `false`:
...
It is important to note that disabling Santander Spring Boot security **does not imply disabling security altogether** since from that moment on, spring security is auto-configured by default.

To completely disable both Santander Spring Boot and Spring security, in addition to disabling Santander Spring Boot security as explained above, you must exclude Spring security by default. as explained above,
certain configuration classes must be excluded to prevent their instantiation. This can be achieved in a `Servlet` application:
...
- **PKMFallbackConnector:** Controls the call to the PKM fallback, whose path shall be obtained as the second value in the list giving value to the `santander.security.connectors.pkm-connector.pkm-endpoint` property.

- **STSFallbackConnector:** Controls the call to the STS fallback, whose path shall be obtained as the second value in the list giving value to the `santander.security.connectors.sts-connector.sts-endpoint` property.
...
### How to configure Santander Spring Boot Authentication WebClient's timeouts

Santander Spring Boot uses preconfigured WebClients to call PKM and STS services. You can customise them via configuration properties. For example:
...
The parameter name is configurable via the `santander.security.auth-query-parameter` property. In case it is not configured, the library will take the default name (`token`) for the parameter.
