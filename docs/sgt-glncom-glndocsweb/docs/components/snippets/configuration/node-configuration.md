...
- **/.gluon/cd/values.yaml**: Configuration with the values to deploy the image.
- **/.gluon/cd/[ENVIRONMENT]/cd.yaml**: Reference to the OAM configuration for any [ENVIRONMENT].
- **/.gluon/cd/[ENVIRONMENT]/values-[ENVIRONMENT].yaml**: specific deployment values for any [ENVIRONMENT].
...
FROM registry.global.ccc.srvb.bo.paas.cloudcenter.corp/produban/nodejs-16-ubi8:1.0.0.RELEASE
...
RUN npm config delete proxy && \
    ...
    curl -k https://nexus.alm.europe.cloudcenter.corp/repository/almmc-san-darwin-sources-raw-releases/nodejs/certs/1.0.0/SANTANDER_CA_CERTS.zip -o /tmp/san-cert.zip && \
    unzip  -o /tmp/san-cert.zip  -d /opt/app/ && \
    unzip  -o /tmp/app.zip  -d /opt/app
ENV NODE_EXTRA_CA_CERTS /opt/app/SANTANDER_CA_CERTS.pem
WORKDIR /opt/app
...
<!--Start Deployment-->
In that set of files, we are going to define the necessary infrastructure references so that the helm chart can deploy the configmap correctly in the configured environment.
...
???+ warning "Setup Github Secrets"
    ...
    Secrets are set up, managed and deployed secrets with [the vault](../../../application/security/security-enablers/hashicorp-vault/journeys/developer/index.md).
...
```yaml linenums="1"
# Default values for darwin micro-node chart.
...
darwin:
  ...
  gitRepo:
  ## @param darwin.technologyVersion. Node version of the image.
  technologyVersion:
...
=== "values-dev.yaml"
    ...
    image:
      ...
      repository: san-narq-ref/darwin-node/sgtnodefastify01
      tag: development
      ## @param image.pullPolicy darwin micro image pull policy
      ## Specify a imagePullPolicy
      ## Defaults to 'Always' if image tag is 'latest', else set to 'IfNotPresent'
...
| `darwin.technologyVersion` | NodeJS version                                                                         |
...
???+ warning "Chart Configuration"
    ...
    For more in-depth information, please visit the Darwin Node Helm chart repository 
    [Darwin NodeJS Helm chart repository](https://registry.global.ccc.srvb.can.paas.cloudcenter.corp/harbor/projects/3068/helm-charts/gln-back-micro-nodejs-chart/versions). 
