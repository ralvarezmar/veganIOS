...
- **Check if npm uses native build**: Check if it is necessary to configure a native compilation.
- **npm build and Sonar scan**: Executes the default commands of npm_BUILD_GOAL: clean verify
- **Container build and push**:
...
??? info "npm CI Image workflow code"
...
jobs:
  call-reusable-workflow:
    name: Integration
    uses: santander-group-shared-assets/gln-workflows/.github/workflows/npm-ci-tbd.yml@v1
    with:
      technology: 'npm'
    secrets: inherit
...
- **Check if npm uses native build**: Check if it is necessary to configure a native compilation.
- **npm build & Sonar scan**: Executes the default commands of npm_BUILD_GOAL: clean verify. If you want to overwrite the npm command you can configure in properties.env.
- **SSDLC Onboarding Check**: Check if the component already exists in Fortify SSC.
- **SAST** Executes the reusable Fortify SAST workflow to perform a SAST scan with Fortify and validate the quality gates.
- **SCA**: Executes the reusable The Sonatype SCA workflow to perform a Sonatype SCA scan and analyze third party components from a repository.
- **Send information to Elasticsearch**:Send data to elasticsearch related with Fortify analysis.
...
jobs:
  call-reusable-workflow:
    name: Release
    uses: santander-group-shared-assets/gln-workflows/.github/workflows/npm-release-tbd.yml@v1
    with:
      technology: 'npm'
    secrets: inherit
