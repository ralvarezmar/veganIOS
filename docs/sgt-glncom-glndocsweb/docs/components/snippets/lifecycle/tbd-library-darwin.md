...
##### Darwin Code Analysis Plugin ArchUnit workflow

- **Setup environment variables**:Load the properties defined in properties.env file and in the configuration project.
- **Darwin Code Analysis Plugin**: Executes the analysis plugin.
...
??? info "Darwin Code Analysis Plugin workflow code"

    ```yaml linenums="1"
    name: Darwin Code Analysis Plugin
    on:
      pull_request:
        branches:
          - development
    ...
    jobs:
      call-reusable-workflow:
        name: Darwin Code Analysis
        uses: santander-group-shared-assets/gln-back-workflows/.github/workflows/darwin-code-validation.yml@v1
        secrets: inherit
    ```
...
???+ note "Availability"
    ...
    If you want to add it to a previously created component you can include a file named `darwin-code-analysis.yml` in
