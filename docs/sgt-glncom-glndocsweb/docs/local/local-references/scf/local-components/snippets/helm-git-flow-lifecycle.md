...
Open a pull request and when approved, the `feature/*` branch is merged into `development`.
...
As in the `feature` branch, when opening the pull request and merged, changes are moved from `development` to `main`.
...
It includes jobs for setting up environment variables in CERT.
- **Release Workflow**: Executes when changes are merged into the `main` branch.
It includes jobs for setting up environment variables and tagging and releasing.
...
- **Deploy in certification**: Deploys the application to the CERT environment.
