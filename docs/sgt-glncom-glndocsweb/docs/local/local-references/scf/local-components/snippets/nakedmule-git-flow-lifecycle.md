...
This uploads the image to the `PRE` and `PRO` environments, generates a tag (e.g., `1.0.0`), and creates a release (e.g., `v1.0.0`). The CD workflow is then initiated for the `PRE` environment.

![Merge to Main](../images/lifecycle/merge-main.png)

#### 6. Manual CD for PRO

The CD workflow for the `PRO` environment is manually triggered from the previous generated tag `1.0.0`.
...
- **Run Deployment in PRE**: This job starts the CD process.
...
The CD process is leveraged to a Python script that deploys the application on the API platform.
It logs into the API to obtain an access token, checks if a configuration file exists and loads it,
verifies if the application already exists to update it or deploys a new one if it doesn't, and finally saves the current application status in a JSON file.
