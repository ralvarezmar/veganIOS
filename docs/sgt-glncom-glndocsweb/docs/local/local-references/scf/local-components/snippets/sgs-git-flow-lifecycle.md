...
It includes jobs for setting up environment variables, security and quality scans, building applications, uploading applications to Nexus and SGS publish locally.
- **Release Workflow**: Executes when changes are merged into the `main` branch.
It includes jobs for setting up environment variables, security and quality scans, building applications, uploading applications to Nexus and SGS publish to the clients.
...
- **Quality Workflow**: Includes jobs for setting up environment variables, building applications, and running Sonar scans.
It includes jobs for setting up environment variables, building applications, tagging and releasing, and uploading applications.
...
- **Application Build and SonarQube**: Compiles the code and runs unit tests, performs code quality analysis.
- **Fortify SAST**: Analyzes vulnerabilities in the source code.
- **Sonatype SCA**: Analyzes vulnerabilities in dependencies.
- **Application Upload**: Uploads the built applications to Nexus.
- **SGS Local Publish**: Publishes in SGS without clients.
...
- **Sonatype SCA**: Analyzes vulnerabilities in dependencies.
- **Application Upload**: Uploads the built applications to Nexus.
- **SGS Local Publish**: Publishes in SGS with clients.
- **Tag and Release**: Generates a tag and creates a release, uploads the image to the `PRE` and `PRO` environments.

### Deployment Rules

- **Certification Environment**: Deployment to the Certification environment will be executed from the CI workflow when it's executed in `development` or `develop` branches.
- **Production Environment**: Deployment to the Production environment will be executed from the Release workflow when it's executed in `main` or `master` branches.

#### Protection Rules

When trying to deploy to the `PRO` environment, protection rules are enforced. Only authorized reviewers can approve the deployment.
...
Just before deploying to the `PRO` environment, the workflow stops and waits for a reviewer to approve the pending deployment.
...
- **Setup Environment Variables**: Sets up necessary environment variables.
- **application Build and Sonar Scan**: Compiles the code, runs unit tests, and performs code quality analysis.
