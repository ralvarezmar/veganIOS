...
The Provenir deployment workflow is responsible for deploying the Provenir application to the specified environment and it consists of two main jobs: prepare and deploy.

The prepare job checks out the repository to the specified Git reference, reads the deployment configuration from deployment.yml or deployment.yaml, converts it to JSON format,
and extracts the deployment nodes for the specified environment, outputting them as a matrix for the next job.

The deploy job checks out the code repository, sets up the runner with the required tools and versions using the provided environment variables, checks out the Provenir binaries from a specified repository,
installs the Provenir application by extracting the binaries, and checks out the Provenir deployment scripts from a specified repository.
Depending on the deployment environment, it either runs the ExportVersion.sh script to export the application version or downloads the artifact from
Nexus and runs the ImportVersion.sh script to import the application version. Finally, it restarts the application pods if the deployment environment is PRE or PRO.

The deployment process is delegated to various scripts (ExportVersion.sh, ImportVersion.sh, RestartPods.sh) that handle specific tasks such as exporting the version,
importing the version, and restarting the pods, ensuring a streamlined and automated deployment process across different environments.
