---
title: SCF Local Component - (SCF) Liquibase Oracle
---
...
![Create Liquibase Oracle Component](images/liquibase/create-liquibase-oracle-component.png)
...
![Complete Liquibase Oracle Component](images/liquibase/complete-liquibase-oracle-component.png)
...
The Liquibase Oracle workflow needs variables to be able to make backups.
...
|---     |---  |---  |---          |
|**DB_URL**| true | The full connection URL for the DB.  | `jdbc:oracle:thin:@[DB_ENDPOINT_NAME]:[Port]/[Database]` |
|**DB_DRIVER**| true | DB driver to use.  | `oracle.jdbc.OracleDriver` |
...
environments:
  CERT:
    DB_URL: jdbc:oracle:thin:@[DB_ENDPOINT_NAME]:[Port]/[Database]
...
- Use the DB backup to perform a rollback to its initial state, prior to tagging the database. The information for this restore is the following:
  ...
  - The Terraform module from CCoE SCF for RDS Oracle [GitHub](https://github.com/santander-group-scfccoe/terraform-aws-iac-ccoescf-rdy) for versions 1.0.2 and later,
    allow to create a RDS Oracle instance from a existing snapshot using snapshot_identifier variable, passing the ARN ID for this snapshot.
  - The user can recreate a existing instance passing this variable allow to recreate the RDS Oracle instance from a different snapshot or create a new instance from the source snapshot.
...
Liquibase is also compatible with PL/SQL scripts.
When using PL/SQL, it is important to specify the `endDelimiter:/` attribute in the changeset and to terminate the script with a `/` character as in the following example.

```sql title="Liquibase PL/SQL script file example" linenums="1"
--changeset ccoe_dev:test_plsql endDelimiter:/
--comment: PL/SQL test
...
--changeset ccoe_dev:test_plsql_procedure endDelimiter:/
--comment: PL/SQL test 2
...
END;
/
--rollback DROP PROCEDURE greetings;
```
...
--changeset ccoe:create_table_country labels:create_table_1
--comment: create create_table_country
CREATE TABLE COUNTRY (
    id int not null,
    name varchar(50) not null
)
...
--changeset ccoe:add_code_table_country labels:add_data_table_country_1
--comment: Add new column code due to project requirements
ALTER TABLE COUNTRY ADD code VARCHAR(10);
