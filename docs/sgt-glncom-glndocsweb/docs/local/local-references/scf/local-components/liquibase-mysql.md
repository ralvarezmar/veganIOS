---
title: SCF Local Component - (SCF) Liquibase MySQL
---
...
![Create Liquibase MySQL Component](images/liquibase/create-liquibase-mysql-component.png)
...
![Complete Liquibase MySQL Component](images/liquibase/complete-liquibase-mysql-component.png)
...
The Liquibase MySQL workflow needs variables to be able to make backups.
...
|---     |---  |---  |---          |
|**DB_URL**| true | The full connection URL for the DB.  | `jdbc:mysql://[DB_ENDPOINT_NAME]:[Port]/[Database]?useUnicode=true&autoReconnect=true&useSSL=false` |
|**DB_DRIVER**| true | DB driver to use.  | `com.mysql.cj.jdbc.Driver` |
...
environments:
  CERT:
    DB_URL: jdbc:mysql://[DB_ENDPOINT_NAME]:[Port]/[Database]?useUnicode=true&autoReconnect=true&useSSL=false
...
- Use the DB backup to perform a rollback to its initial state, prior to tagging the database. The information for this restore is the following:
  ...
  - The Terraform module from CCoE SCF for RDS MySQL [GitHub](https://github.com/santander-group-scfccoe/terraform-aws-iac-ccoescf-rdy) for versions 1.0.2 and later,
    allow to create a RDS MySQL instance from a existing snapshot using snapshot_identifier variable, passing the ARN ID for this snapshot.
  - The user can recreate a existing instance passing this variable allow to recreate the RDS MySQL instance from a different snapshot or create a new instance from the source snapshot.
...
- If we define one or more tags inside our code, we could also rollback to them, allowing us to rollback some of the changes applied.ur code, we could also rollback to them, allowing us to rollback some of the changes applied.
...
--comment: create create_schema
CREATE SCHEMA IF NOT EXISTS demo_schema;
--rollback DROP SCHEMA IF EXISTS demo_schema;

--changeset author:changeset_id_2 labels:changeset_label_2
--comment: create create_table_status
CREATE TABLE IF NOT EXISTS demo_schema.demo_status_table (table_name varchar PRIMARY KEY, sequence bigint);
--rollback DROP TABLE IF EXISTS demo_schema.demo_status_table;

--changeset author:changeset_id_2 labels:changeset_label_3
--comment: insert
INSERT INTO demo_schema.demo_status_table (table_name, sequence) VALUES ('test', 1);
...
--changeset ccoe:create_table_status labels:create_table_1
--comment: create create_table_status
CREATE TABLE IF NOT EXISTS demo_schema.demo_status_table (
...
--changeset ccoe:add_date_table_status labels:add_data_table_status_1
--comment: Add new column date due to project requirements
ALTER TABLE demo_schema.demo_status_table ADD date datetime;
--rollback ALTER TABLE demo_schema.demo_status_table DROP COLUMN date;
```
