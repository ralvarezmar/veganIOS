---
title: SCF Local Component - (SCF) Liquibase PostgreSQL
---
...
![Create Liquibase PostgreSQL Component](images/liquibase/create-liquibase-postgresql-component.png)
...
![Complete Liquibase PostgreSQL Component](images/liquibase/complete-liquibase-postgresql-component.png)
...
The Liquibase Postgresql workflow needs variables to be able to make backups.
...
|---     |---  |---  |---          |
|**DB_URL**| true | The full connection URL for the DB.  | `jdbc:postgresql://[DB_ENDPOINT_NAME]/[DB_NAME]` |
|**DB_DRIVER**| true | DB driver to use.  | `org.postgresql.Driver` |
...
environments:
  CERT:
    DB_URL: jdbc:postgresql://[DB_ENDPOINT_NAME]/[DB_NAME]
    ...
    AWS_INSTANCE_ID:
    # Only change if your requested role has a different name from the default 'GitHubActions-Terraform'
    AWS_ROLE_NAME: 'GitHubActions-Terraform'
  PRE:
    DB_URL: jdbc:postgresql://[DB_ENDPOINT_NAME]/[DB_NAME]
    DB_DRIVER: org.postgresql.Driver
    DB_TYPE: AWS
...
- Use the DB backup to perform a rollback to its initial state, prior to tagging the database. The information for this restore is the following:
  ...
  - The Terraform module from CCoE SCF for RDS Aurora [GitHub](https://github.com/santander-group-scfccoe/terraform-aws-iac-ccoescf-rdsaurora) for versions 1.0.7 and later,
    allow to create a RDS Aurora Cluster from a existing snapshot using snapshot_identifier variable, passing the ARN ID for this snapshot.
  - The user can recreate a existing cluster passing this variable allow to recreate the RDS Postgresql cluster from a different snapshot or create a new cluster from the source snapshot.
