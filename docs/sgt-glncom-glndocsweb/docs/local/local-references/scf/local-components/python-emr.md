---
title: (SCF) Python EMR
---

This base component template serves as a comprehensive guide for building Python components.
The Python EMR component allows you to upload to an S3 bucket not only the relevant zipped folder and the typical Python (.py) entry points, but also a virtual environment and one or more uncompressed configuration folders.
This aims to cover the usual needs of the EMR use case.

In this documentation, users will find detailed instructions, best practices, and examples to effectively utilize the Python EMR component workflows for their projects.
This includes guidance on setting up the project, configuring essential files, managing dependencies, and performing the upload process to an S3 bucket.

Whether you are starting from scratch or integrating the Python EMR component into an existing project, this guide will provide the necessary steps to ensure a smooth and efficient development and deployment process.
...
Additionally, the runner is configured to use Java OpenJDK 17, so ensure compatibility with the PySpark version used, if applicable.
...
* `requirements.txt`: file where the libraries and dependencias of your project are listed
...
```yaml
    CONFIG_S3: "my-bucket"
    CONFIG_TARGET_FOLDER: "my/target/folder"
...
environments:
  DEV:
    ...
    COMPRESS_FILE_NAME: "source-code"                 # Name of the compressed file that will be created and uploaded to s3
  ...
  PRE:
    ...
    VENV_S3: "my-bucket"
    VENV_TARGET_FOLDER: "my/folder2"

    CONFIG_FOLDER_NAME: "JSONCONFIG"
    CONFIG_S3: "my-bucket"
    CONFIG_TARGET_FOLDER: "my/folder3"
...
???+ warning "Note"
      ...
      For **PySpark projects** that require a specific Java version to run, add the key **JAVA_VERSION** to the properties.env file with the required version (for example: JAVA_VERSION=11).
      Available Java versions can be found at: https://gluon.gs.corp/community/docs/latest/getting-started/company-management/technical-requirements/ephemeral-runners/flavours/#java-versions
