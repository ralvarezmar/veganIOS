---
title: SCF Local Component - (SCF) Liquibase Snowflake
---
...
![Create Liquibase Snowflake Component](images/liquibase/create-liquibase-snowflake-component.png)
...
![Complete Liquibase Snowflake Component](images/liquibase/complete-liquibase-snowflake-component.png)
...
|---     |---  |---  |---          |
|**DB_URL**| true | The full connection URL for the DB.  | `jdbc:snowflake://[ACCOUNT_URL]/?db=[DEFAULT_DB]&warehouse=[DEFAULT_WAREHOUSE]&schema=[DEFAULT_SCHEMA]&useProxy=true&proxyHost=proxyappcan.cloudcenter.corp&proxyPort=8080` |
|**DB_DRIVER**| true | DB driver to use.  | `net.snowflake.client.jdbc.SnowflakeDriver` |
|**DB_TYPE**| true | Type of the DB. | `SNOWFLAKE` |
...
|**LIQUIBASE_SCHEMA_NAME**| true | The schema name where liquibase creates the changelog tables. | `liquibase` |
...
environments:
  CERT:
    DB_URL: jdbc:snowflake://[ACCOUNT_URL]/?db=[DEFAULT_DB]&warehouse=[DEFAULT_WAREHOUSE]&schema=[DEFAULT_SCHEMA]&useProxy=true&proxyHost=proxyappcan.cloudcenter.corp&proxyPort=8080
    DB_DRIVER: net.snowflake.client.jdbc.SnowflakeDriver
    DB_TYPE: SNOWFLAKE
...
- Snowflake does not support database backup or taking snapshots. But it has features like data retention and fail-safe.
...
- Fail-Safe, provides a (non-configurable) 7-day period during which historical data may be recoverable by Snowflake. This period starts immediately after the Time Travel retention period ends.
  Note, however, that a long-running Time Travel query will delay moving any data and objects (tables, schemas, and databases) in the account into Fail-safe, until the query completes.
...
--rollback DROP SCHEMA IF EXISTS demo_db.demo_schema;

--changeset author:changeset_id_2 labels:changeset_label_2
--comment: create create_table_status
CREATE TABLE IF NOT EXISTS demo_db.demo_schema.demo_status_table (table_name varchar, status varchar(50));
--rollback DROP TABLE IF EXISTS demo_db.demo_schema.demo_status_table;

--changeset author:changeset_id_2 labels:changeset_label_3
--comment: insert
INSERT INTO demo_db.demo_schema.demo_status_table (table_name, status) VALUES ('test', 'ok');
...
--comment: create create_table_status
CREATE TABLE IF NOT EXISTS demo_db.demo_schema.demo_status_table (
...
ALTER TABLE demo_db.demo_schema.demo_status_table ADD date datetime;
--rollback ALTER TABLE demo_db.demo_schema.demo_status_table DROP COLUMN date;
```
