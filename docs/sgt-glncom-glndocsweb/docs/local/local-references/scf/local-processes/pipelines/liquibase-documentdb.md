---
title: SCF Liquibase DocumentDB Adaptation Guide
---

This page aims to assist in the migration of components that previously relied on the ALM Multicloud pipeline `pipelineForLiquibase` in Jenkins.
The guide focuses on the migration and adaptation of the GitHub repository files to comply with Gluon.
Full information about to use these component templates can be found in the following documentations: [(SCF) Liquibase DocumentDB](https://gluon.gs.corp/community/docs/latest/local/local-references/scf/local-components/liquibase-documentdb/).
...
## How to identify Liquibase DocumentDB pipelines

In all liquibase pipelines, to identify if the pipeline uses DocumentDB or not, the variable **DB_DRIVER**must has the value **liquibase.ext.documentdb.database.DocumentDbClientDriver**
defined and the variable **DB_TYPE**must has the value **AWS** set.
...
In order to create the component go to your application in Gluon portal > Components > New Component and select `(SCF) Liquibase DocumentDB`.

![Create Component](images/liquibase/create-documentdb.png)
...
The folder **env** which contains all liquibase script files must be renamed to **src**.
...
![config](images/generic/ci-config.png)
...
- The fields `MAX_DB_BACKUPS` and `DISABLE_BACKUP` must be filled using the same value from file ci-config.groovy.

![Properties](images/liquibase/properties.png)
...
![Deployment](images/liquibase/deployment-documentdb.png)

The value of the **DB_URL**must be modified, this is done by appending the following line at the end of the original endpoint.

- `&tlsCAFile=global-bundle.pem`

### Secrets Configuration

The deployment requires user credentials set as secrets in your repository to be able to authenticate against the database.

To do so, go to `Settings > Code and automation > Environments` select the environment (certification, preproduction and production), and add the following secrets, `DB_PASSWORD` and `DB_USERNAME`, at “Environment secrets“ section.

![Secrets](images/liquibase/secrets.png)

To enable the database backups, environment variables must be set, in this case, the AWS account id, to do so, go to `Settings > Code and automation > Environments` select the environment
(certification, preproduction and production), and add the following variable, `AWS_ACCOUNT_ID`, at “Environment variables“ section.

![variables](images/liquibase/variables.png)
