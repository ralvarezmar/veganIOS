---
title: SCF Liquibase MYSQL Adaptation Guide
---
...
Full information about to use these component templates can be found in the following documentations: [(SCF) Liquibase MYSQL](https://gluon.gs.corp/community/docs/latest/local/local-references/scf/local-components/liquibase-mysql/).
...
## How to identify Liquibase MySQL pipelines

In all liquibase pipelines, to identify if the pipeline uses MySQL or not, the variable **DB_DRIVER**must has the value **com.mysql.cj.jdbc.Driver** defined and the variable **DB_TYPE**must has the value **AWS** set.
...
In order to create the component go to your application in Gluon portal > Components > New Component and select `(SCF) Liquibase MYSQL`.

![Create Component](images/liquibase/create-mysql.png)
...
![Deployment](images/liquibase/deployment-mysql.png)

**In case the connection fails**,****the value of the **DB_URL**must be modified, this is done by appending the following line at the end of the original endpoint.
