---
title: SCF Python Virtual Environment Adaptation Guide
---

This page aims to assist in the migration of components that previously relied on the ALM Multicloud pipeline `pipelineForPythonVirtualEnvironment` in Jenkins.
The guide focuses on the migration and adaptation of the GitHub repository files to comply with Gluon.
Full information about how to use this component template can be found in [documentation](https://gluon.gs.corp/community/docs/latest/local/local-references/scf/local-components/python-venv/).
...
In order to create the component go to your application in Gluon portal > Components > New Component and select `(SCF) Python Virtual Environment`.

![Create Component](images/python/venv-create-component.png)
...
environments:
  DEV:
    ...
    S3BUCKET_NAME: "testBucket"                    # s3://<S3BUCKET_NAME> - It must previously exist
    TARGET_FOLDER: "testFolder"                    # s3://<S3BUCKET_NAME>/<TARGET_FOLDER> - If it doesn't exist, it will be created.
...
This file contains metadata about the python project.
You can set the values for the different parameters, taking into account that each time you want to deploy to PRE or PRO environment, the version must be higher than the previous one deployed to PRE or PRO environment.
...
This file is located inside .gluon/ci directory. The only parameter that should be modified is PYTHON_REQUIRES, adjusting the value to the python version your project needs.
