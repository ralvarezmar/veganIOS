---
title: SCF Liquibase Athena Snowflake Guide
---
...
Full information about to use these component templates can be found in the following documentations: [(SCF) Liquibase Snowflake](https://gluon.gs.corp/community/docs/latest/local/local-references/scf/local-components/liquibase-snowflake/).
...
## How to identify Liquibase Snowflake pipelines

In all liquibase pipelines, to identify if the pipeline uses Snoflake or not, the variable **DB_TYPE**must has the value **SNOWFLAKE**set.
...
In order to create the component go to your application in Gluon portal > Components > New Component and select `(SCF) Liquibase Snowflake`.

![Create Component](images/liquibase/create-snowflake.png)
...
![Deployment](images/liquibase/deployment-snowflake.png)

The value of the **DB_URL**must be modified, this is done by appending at the end or modifying the existing proxy configuration using the following line.

- `&useProxy=true&proxyHost=proxyappcan.cloudcenter.corp&proxyPort=8080`
