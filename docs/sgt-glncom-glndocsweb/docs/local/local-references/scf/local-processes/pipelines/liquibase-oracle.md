---
title: SCF Liquibase Oracle Adaptation Guide
---
...
Full information about to use these component templates can be found in the following documentations: [(SCF) Liquibase Oracle](https://gluon.gs.corp/community/docs/latest/local/local-references/scf/local-components/liquibase-oracle/).
...
## How to identify Liquibase Oracle pipelines

In all liquibase pipelines, to identify if the pipeline uses Oracle or not, the variable **DB_DRIVER**must has the value **oracle.jdbc.OracleDriver** defined and the variable **DB_TYPE**must has the value **AWS** set.
...
In order to create the component go to your application in Gluon portal > Components > New Component and select `(SCF) Liquibase Oracle`.

![Create Component](images/liquibase/create-oracle.png)
...
![Deployment](images/liquibase/deployment-oracle.png)
