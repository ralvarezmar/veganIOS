---
title: SCF Liquibase Postgres Adaptation Guide
---
...
Full information about to use these component templates can be found in the following documentations: [(SCF) Liquibase PostgreSQL](https://gluon.gs.corp/community/docs/latest/local/local-references/scf/local-components/liquibase-postgresql/).
...
## How to identify Liquibase PostgreSQL pipelines

In all liquibase pipelines, to identify if the pipeline uses PostgreSQL or not, the variable **DB_DRIVER**must has the value **org.postgresql.Driver** defined and the variable **DB_TYPE**must has the value **AWS** set.
...
In order to create the component go to your application in Gluon portal > Components > New Component and select `(SCF) Liquibase Postgres`.

![Create Component](images/liquibase/create-postgres.png)
...
![Deployment](images/liquibase/deployment-postgre.png)
