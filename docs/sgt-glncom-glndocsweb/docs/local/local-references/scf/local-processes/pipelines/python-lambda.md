---
title: SCF Python Lambda Adaptation Guide
---

This page aims to assist in the migration of components that previously relied on the ALM Multicloud pipeline `pipelineForPythonLambda` in Jenkins.
The guide focuses on the migration and adaptation of the GitHub repository files to comply with Gluon.
Full information about how to use this component template can be found in [documentation](https://gluon.gs.corp/community/docs/latest/local/local-references/scf/local-components/python-lambda/).
...
In order to create the component go to your application in Gluon portal > Components > New Component and select `(SCF) Python Lambda`.

![Create Component](images/python/lambda-create-component.png)
...
environments:
  DEV:
    ...
    LAMBDA_NAME: "pythonlambdatest"                   # The name of your Lambda resource.
    LAMBDA_HANDLER: "lambda_function.lambda_handler"  # The predefined value is the one set, corresponding to python_file_name.handler_function_name.
    OVERWRITE: 0                                      # Set to 1 to replace everything located at s3://<S3BUCKET_NAME>/<TARGET_FOLDER>
    IS_PUBLIC: 0
    AWS_REGION: "eu-west-1"
    AWS_ROLE_NAME: ""                                 # Name of the AWS Role ('GitHubActions-Terraform' by default)

  PRE:
    AWS_ACCOUNT_ID: 001122334455
    S3BUCKET_NAME: "my-bucket"
    TARGET_FOLDER: "test"
    LAMBDA_NAME: "pythonlambdatest"
