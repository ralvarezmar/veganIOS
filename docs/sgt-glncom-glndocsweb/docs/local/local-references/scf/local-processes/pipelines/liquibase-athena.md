---
title: SCF Liquibase Athena Adaptation Guide
---
...
Full information about to use these component templates can be found in the following documentations: [(SCF) Liquibase Athena](https://gluon.gs.corp/community/docs/latest/local/local-references/scf/local-components/liquibase-athena/).
...
## How to identify Liquibase Athena pipelines

In all liquibase pipelines, to identify if the pipeline uses Athena or not, the variable **DB_TYPE** in the `deployment.yml` file must has the value **ATHENA**defined.
...
In order to create the component go to your application in Gluon portal > Components > New Component and select `(SCF) Liquibase Athena`.

![Create Component](images/liquibase/create-athena.png)
...
![Deployment](images/liquibase/deployment-athena.png)
...
To enable the deployment, environment variables must be set, in this case, the AWS account id, to do so, go to `Settings > Code and automation > Environments`select the environment
