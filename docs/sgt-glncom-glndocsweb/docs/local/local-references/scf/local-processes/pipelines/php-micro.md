---
title: SCF PHP Microservice Adaptation Guide
---

This page aims to assist in the migration of components that previously relied on the ALM Multicloud pipeline `pipelineForDrupal` in Jenkins.
The guide focuses on the migration and adaptation of the GitHub repository files to comply with Gluon.
Full information about yo use this component template can be found in [documentation](https://gluon.gs.corp/community/docs/latest/local/local-references/scf/local-components/php-micro/).
...
In order to create the component go to your application in Gluon portal > Components > New Component and select `(SCF) PHP Microservice`.

![Create Component](images/php/create-component.png)
...
📂.github
 ...
 | ┣ 📜php-cd-image.yml
 | ┣ 📜php-rc-image.yml
 | ┣ 📜php-security-image.yml
 | ┣ 📜php-quality-image.yml
 | ┣ 📜update-component-workflow.yml
 | ┗ 📜php-version-validation.yml
 ┗ 📜CODEOWNERS
📂.gluon
 ┣ 📂ci
 | ┗ 📜properties.env
...
- Set `IMAGE_DEPLOY_TYPE` to the desired deployment type: "helm" or “ose3” for Openshift.
Note that when using Openshift a Token must be set within your repository secrets, check the [Secrets Configuration](https://san-sc-we.atlassian.net/wiki/spaces/SCFCCOE/pages/edit-v2/682524691#%F0%9F%93%91--Secrets-Configuration) section for more info.
...
![Properties](images/php/deployment.png)
...
EKS deployments don’t require setting any credentials as secrets in your repository as authentication is done using roles. **Only in Openshift**deployment cases a Token is required for each environment as GitHub Repository Secrets.

To do so, go to `Settings > Security > Secrets and variables > Actions` and add the following secrets at “Repository secrets” level.

![Secrets](images/php/secrets.png)
...
**Warning:**Ensure you are using IRSA when configuring your AWS SDK for Amazon EKS. This is crucial for secure and efficient access management.
It is essential for replacing traditional user/password methods when consuming AWS resources from within your pods.
