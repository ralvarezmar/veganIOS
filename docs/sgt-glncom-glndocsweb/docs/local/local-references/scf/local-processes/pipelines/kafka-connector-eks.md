---
title: SCF Kafka Connector on EKS Adaptation Guide
---

This page aims to assist in the migration of components that previously relied on the ALM Multicloud pipeline `pipelineForKafkaConnectorEKS` in Jenkins.
The guide focuses on the migration and adaptation of the GitHub repository files to comply with Gluon.
Full information about to use these component templates can be found in the following documentations: [(SCF) Kafka Connect Connectors EKS](https://gluon.gs.corp/community/docs/latest/local/local-references/scf/local-components/kafka-connect-connectors-eks/).

## Prerequisites: AWS Credentials Request

For this pipeline to work correctly, you need to request a role + OIDC.
Note that this template is cataloged as a `Non Microservice components > Kafka`, detailed information to make the request can be found [here](../../support/credentials/data.md).
...
In order to create the component go to your application in Gluon portal > Components > New Component and select `(SCF) Kafka Connector EKS`.

![Create Component](images/kafka/create-component.png)
...
The folder **env** which contains all **Kafka Connect Connectors** script files must be renamed to **src**.
...
![Deployment](images/kafka/deployment.png)
...
![Secrets](images/kafka/secrets.png)
