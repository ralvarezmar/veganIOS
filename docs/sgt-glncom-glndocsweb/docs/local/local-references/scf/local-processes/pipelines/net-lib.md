---
title: SCF .NET Library Adaptation Guide
---

This page aims to assist in the migration of components that previously relied on the ALM Multicloud pipeline `pipelineForNETCORELib` in Jenkins.
...
In order to create the component go to your application in Gluon portal > Components > New Component and select `(SCF) .NET Library`.

![Create Component](images/net/net-lib-create.png)
...
![Create Component](images/net/net-lib-create-cont.png)
...
In this caso there are nothing to copy from ci-config,The key `APP_NAME` in the `ci-config.groovy` file is deprecated.
That variable determined the name of the nuget package that was going to be uploaded to nexus, now the name of the package is generated automatically when executing the command nuget pack.

There is a new feature that is `PROJECT_PATH` key, which by default will take the `.sln` file from the root of the repository about which it will be compiled and packaged, but you can specify any path to any `.sln or .csproj` file in the repository.
...
![Ci Config](images/generic/version-json.png)
