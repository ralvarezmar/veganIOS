---
title: SCF Liquibase MSSQL Adaptation Guide
---
...
Full information about to use these component templates can be found in the following documentations: [(SCF) Liquibase MSSQL](https://gluon.gs.corp/community/docs/latest/local/local-references/scf/local-components/liquibase-mssql/).
...
## How to identify Liquibase MSSQL pipelines

In all liquibase pipelines, to identify if the pipeline uses MSSQL or not, the variable **DB_DRIVER**must has the value **com.microsoft.sqlserver.jdbc.SQLServerDriver** defined and the variable **DB_TYPE**must has the value **AWS** set.
...
In order to create the component go to your application in Gluon portal > Components > New Component and select `(SCF) Liquibase MSSQL`.

![Create Component](images/liquibase/create-mssql.png)
