...
Not all mavenized artifacts are susceptible to review by the Sonar and Fortify tools.
...
- **Send information to Elasticsearch**: Send data to elasticsearch related with SAST and SCA analysis.
- **Maven artifact upload**: Then execute the command mvn clean package -Dmaven.test.skip=true to build the application artifact.
