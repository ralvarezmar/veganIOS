## API Release deployment (api-release-deployment.yml)
...
* `Calling quality workflow:` This job calls the quality workflow.
  It executes a complete workflow with 'Setup environment variables',
  'Get Definition File', 'API validation', 'Linter validation' and  
  'Send data to Elastic' jobs. More details [here](./api-quality-deployment.md).
...
* `Calling cd workflow` This job calls the CD workflow in order
  to perform the deployment indicated by the RELEASE_DEPLOYMENT
...
[Apis release deployment workflow link](https://github.com/santander-group-shared-assets/gln-workflows/blob/main/.github/workflows/api-release-deployment.yml)
